...
Two smaller, honest divergences remain in the union payload, both size-only (not
correctness): the §2 **all-ones `lanes`-child omission** is not applied (bitmaps
are always stored — the reader handles both, so reconstruction stays SHA-exact;
omission needs the per-revision live-lane count threaded into the encoder), and
