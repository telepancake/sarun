...
//! (nowiki/pre bodies). `escape_body` NORMALIZES character references the
//! way MediaWiki's `Sanitizer::normalizeCharReferences` does — a defined
//! named entity becomes its decimal numeric form (`&eacute;` → `&#233;`),
//! `amp`/`lt`/`gt`/`quot` stay in word form, numeric references are
//! re-emitted canonically, and everything else (a bare `&`, an undefined
//! or semicolon-less name, an out-of-range codepoint) is escaped to
//! `&amp;…`.
...
/// Body-text escape: NORMALIZES a character reference at each `&`
/// ([`normalize_entity_at`]) and escapes every other `&`. Does not touch
/// `"` (legal in body text).
pub fn escape_body(s: &str) -> String {
    ...
    while i < b.len() {
        match b[i] {
            ...
            b'&' => {
                if let Some((len, norm)) = normalize_entity_at(&s[i..]) {
                    out.push_str(&norm);
                    i += len;
                } else {
                    out.push_str("&amp;");
...
/// Attribute-value escape: like [`escape_body`] (references normalized)
...
/// MediaWiki `Sanitizer::normalizeCharReferences` for the reference that
/// begins at `s[0]=='&'`. Returns `(bytes_consumed, normalized)` for a
/// reference MediaWiki keeps, or `None` when the `&` is not a defined
/// reference — the caller then emits `&amp;` and re-scans the tail as
/// literal text, exactly reproducing `normalizeEntity`'s `&amp;$name`.
///
///   * `&#233;` / `&#xE9;` — numeric: re-emitted canonically (decimal
///     stays decimal, hex stays lowercase hex) when the codepoint is a
///     valid HTML5+XML character, else `None`.
///   * `&amp;`/`&lt;`/`&gt;`/`&quot;` — kept in word form.
///   * `&eacute;` — a defined HTML5 named entity → its decimal numeric
///     form (`&#233;`); a multi-codepoint entity expands to several.
///   * `&xacute;`, `&ampamp;`, bare `&` — `None`.
fn normalize_entity_at(s: &str) -> Option<(usize, String)> {
    let b = s.as_bytes();
    if b.first() != Some(&b'&') {
        return None;
    }
    if b.get(1) == Some(&b'#') {
        let hex = matches!(b.get(2), Some(b'x') | Some(b'X'));
        let mut i = if hex { 3 } else { 2 };
        let start = i;
        while i < b.len()
        ...
        return Some((i + 1, norm));
    }
    // Named: `&` + alnum name + `;` (the captured name INCLUDES the `;`,
    // matching MediaWiki's CHAR_REFS_REGEX and its keyed lookup).
    let mut i = 1;
    ...
    let name = &s[1..=i]; // e.g. "eacute;"
    let consumed = i + 1;
    match name {
        "amp;" => Some((consumed, "&amp;".to_string())),
        "lt;" => Some((consumed, "&lt;".to_string())),
        "gt;" => Some((consumed, "&gt;".to_string())),
        "quot;" => Some((consumed, "&quot;".to_string())),
        _ => named_entity_numeric(name).map(|v| (consumed, v.to_string())),
    }
}

/// A codepoint valid in both HTML5 and XML (MediaWiki
...
/// Look up a semicolon-terminated HTML5 named entity (e.g. `"eacute;"`)
/// and return its precomputed decimal numeric-reference form. `None` for
/// an undefined name.
fn named_entity_numeric(name_with_semicolon: &str) -> Option<&'static str> {
    NAMED_ENTITIES
        .binary_search_by(|(k, _)| (*k).cmp(name_with_semicolon))
        .ok()
...
/// A repeated attribute keeps the LAST value at its FIRST position —
/// MediaWiki's `fixTagAttributes` dedup (`class="error" class="awesome"`
/// → `class="awesome"`).
pub fn sanitize_attrs(raw: &str) -> String {
    ...
    for (name, value) in parse_attrs(raw) {
        ...
        if let Some(pos) = names.iter().position(|n| n == &name) {
            values[pos] = value; // last value wins, first position kept
        } else {
            names.push(name);
            values.push(value);
        }
...
// GENERATED from WHATWG entities.json (html.spec.whatwg.org/entities.json).
// HTML5 named character references (semicolon-terminated), each mapped to
// its decimal numeric-reference form, matching MediaWiki
// Sanitizer::normalizeEntity (HTMLData::$namedEntityTranslations). The XML
// predefined amp/lt/gt/quot are handled separately (kept in word form) and
// are intentionally absent here. Sorted by name for binary search.
