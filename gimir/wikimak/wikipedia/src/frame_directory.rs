...
///
/// Cost: one 64-byte header read and one seek over each compressed frame,
/// exactly `128 + 64 * frames` output bytes, one input descriptor at a time,
/// and constant retained memory.
...
impl FrameDirectoryWriter {
    fn new(path: &Path, identity: [u8; 32]) -> Result<Self> {
        let parent = path
            .parent()
    ...
    fn push(&mut self, entry: FrameDirectoryEntry) -> Result<()> {
        validate_entry(entry)?;
        if self.frames == 0 {
    ...
    fn finish(mut self) -> Result<FrameDirectorySummary> {
