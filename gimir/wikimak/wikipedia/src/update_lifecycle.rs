...
pub(crate) enum UpdateEvent {
    ...
    NewReaderOpenedBase,
    NewReaderOpenedCandidate,
}

pub(crate) const ALL_UPDATE_EVENTS: [UpdateEvent; 26] = [
    ...
    UpdateEvent::ExistingReaderObserved,
    UpdateEvent::NewReaderOpenedBase,
    UpdateEvent::NewReaderOpenedCandidate,
];
...
pub(crate) fn transition(
    ...
    if state == Cleaned {
        return match event {
            Cleanup | ExistingReaderObserved | NewReaderOpenedCandidate => NoOp,
            StaleReceipt | ForeignReceipt | ResumeRequested | RetryRequested
            | DuplicateReceipt | NewReaderOpenedBase => {
    ...
    match event {
        PublishPlan => match state {
            ...
            Planned | TailReady | BasePreserved | ApplyingRanges | RangesApplied
        ...
        PublishRangePlan => linear_event(state, BasePreserved, ApplyingRanges),
        PublishRange => match state {
            Unplanned | Planned | TailReady | BasePreserved => Reject(Rejection::OutOfOrder),
            ApplyingRanges => NoOp,
        ...
        DiscoveryFailed | SourceGapDetected | WorkerFailed | CancelRequested
        | ProcessCrashed => NoOp,
        ...
        InstallInterrupted => match state {
            ...
            Unplanned | Planned | TailReady | BasePreserved | ApplyingRanges
            | RangesApplied | CandidateComplete | Committed => {
                Reject(Rejection::OutOfOrder)
            }
            Cleaned => unreachable!(),
        },
        CleanupFailed => match state {
            Committed => NoOp,
            Unplanned | Planned | TailReady | BasePreserved | ApplyingRanges
            | RangesApplied | CandidateComplete | IndexReady | Installed => {
                Reject(Rejection::OutOfOrder)
            }
            Cleaned => unreachable!(),
        },
        ExistingReaderObserved => NoOp,
        NewReaderOpenedBase => match state {
            Unplanned | Planned | TailReady | BasePreserved | ApplyingRanges
            | RangesApplied | CandidateComplete | IndexReady => NoOp,
            Installed | Committed => Reject(Rejection::OutOfOrder),
            Cleaned => unreachable!(),
        },
        NewReaderOpenedCandidate => match state {
            Unplanned | Planned | TailReady | BasePreserved | ApplyingRanges
            | RangesApplied | CandidateComplete | IndexReady => {
                Reject(Rejection::OutOfOrder)
            }
            Installed | Committed => NoOp,
            Cleaned => unreachable!(),
        },
    }
}
...
impl UpdateState {
    ...
    pub(crate) fn next_action(&self) -> UpdateAction {
        match self.phase() {
            ...
            UpdatePhase::BasePreserved => UpdateAction::PublishRangePlan,
...
impl UpdatePaths {
    ...
    pub(crate) fn base_range_frame_directory(&self, segment_id: &str) -> PathBuf {
        self.root
            .join("ranges")
            .join("base-frame-directories")
            .join(format!("{segment_id}.swframe"))
    }
...
fn validate_tail(
    ...
    if !tail.complete {
        return Err(invalid(&path, "tail receipt does not record clean completion"));
    }
    ...
    {
        return Err(invalid(&path, "tail frame-directory format is not canonical"));
    }
    ...
    match &receipt.selection {
        RangeSelection::Unchanged {
                ...
                || receipt.title_projection_records != 0
            {
                return Err(invalid(
                    &path,
                    "unchanged range records archive I/O",
                ));
            }
            Ok(SelectedSegment {
        ...
        RangeSelection::Replaced {
            ...
            match (
                ...
                _ => {
                    return Err(invalid(
                        &path,
                        "title projection receipt is inconsistent",
                    ))
                }
            }
            Ok(SelectedSegment {
...
fn validate_inventory(
    ...
    if data.len() != inventory.segments.len()
        || data.iter().zip(&inventory.segments).any(|(segment, selected)| {
...
pub(crate) fn inspect_update(
    ...
    let Some(inventory) =
        read_receipt::<CandidateInventoryReceipt>(&paths.candidate_inventory())?
    else {
        require_link(
            &paths.range_plan(),
            installed_generation_id,
    ...
    validate_inventory(paths, &plan, &tail, &selected, &inventory)?;
    phase = observed_transition(
    ...
    let Some(generation) =
        read_receipt::<PreparedGenerationReceipt>(&paths.prepared_generation())?
    else {
        require_link(
            &paths.candidate_inventory(),
            installed_generation_id,
    ...
    validate_prepared_generation(paths, &plan, &generation)?;
...
mod tests {
    ...
    fn source_and_plan() -> (crate::direct::UpdateSourcePlan, UpdatePlanReceipt) {
        let base_generation_id =
            crate::generation::GenerationId::from_plan_bytes(b"test base");
        ...
        source.generation_id =
            crate::generation::GenerationId::from_plan_bytes(&generation);
    ...
    fn every_phase_event_pair_has_an_explicit_transition_classification() {
        ...
        let expected = [
            ...
            [
                ...
                NoOp,
                NoOp,
                Impossible(RangeAfterFinalRange),
                NoOp,
                Advance(CandidateComplete),
                Reject(Rejection::OutOfOrder),
                Reject(Rejection::OutOfOrder),
                Reject(Rejection::OutOfOrder),
                Reject(Rejection::OutOfOrder),
        ...
        let expected = [
            [
                NoOp, NoOp, NoOp, NoOp, NoOp, r, r, r, r, r, r, r, NoOp,
                NoOp, r,
            ],
            ordinary,
            ordinary,
            ...
            ordinary,
            [
                NoOp, NoOp, NoOp, NoOp, NoOp, NoOp, NoOp, NoOp, r, r, NoOp,
                r, NoOp, NoOp, r,
            ],
            [
                NoOp, NoOp, NoOp, NoOp, NoOp, NoOp, NoOp, NoOp, r, r, NoOp,
            ...
            [
                NoOp, NoOp, NoOp, NoOp, NoOp, NoOp, NoOp, NoOp, r, r, r,
                NoOp, NoOp, r, NoOp,
            ],
            [x, x, x, x, x, r, r, r, r, r, x, x, NoOp, r, NoOp],
        ];
        for (state_index, state) in phases.into_iter().enumerate() {
            for (event_index, event) in events.into_iter().enumerate() {
                assert_eq!(
    ...
    fn tail_cursor_resume_replays_only_its_single_bounded_frame() {
        ...
        let identity =
            crate::generation::GenerationId::from_plan_bytes(b"cursor-test");
