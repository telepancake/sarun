#!/usr/bin/env python3
"""Assemble vendor/ from pinned pristine upstreams + patch series.

Sources are pinned in vendor.toml; the project delta per crate lives in
vendor-patches/<crate>/ as {files, series, *.patch}. This script
...
with `git apply`. The result in vendor/ is not tracked by git.

Idempotent: a crate is reassembled only when its manifest entry or patch dir
changed (content stamp in vendor/<crate>/.stamp). `--force` rebuilds all.
"""
...
PATCHES = os.path.join(PROJECT, "vendor-patches")
CACHE = os.path.join(PROJECT, ".vendor-cache")
...
def assemble(name, entry, root):
    ...
    stage = tempfile.mkdtemp(prefix="bumba-vendor-")
    try:
        for fl in files:
        ...
        for p in series:
            ...
            if r.returncode != 0:
                sys.stderr.write(
                    ...
                    f"  or edit the hunks in vendor-patches/{name}/{p}\n"
                    f"  directly, and rerun `make vendor`. Do not fold patches\n"
                    f"  together. See vendor-patches/README.md.\n")
                sys.exit(1)
...
def load_manifest():
    with open(os.path.join(PROJECT, "vendor.toml"), "rb") as f:
...
def assemble_baseline(name, drop_last=False):
    ...
    diffs the WORKING tree in vendor/<name> against this baseline."""
    manifest = load_manifest()
    entry = manifest[name]
    entry["_name"] = name
    ...
    stage = tempfile.mkdtemp(prefix="bumba-vendor-base-")
    for fl in files:
        src = os.path.join(root, fl)
        dst = os.path.join(stage, fl)
        copy_selected_file(src, dst, fl)
...
def working_delta(name, baseline):
    """`git diff` of the working tree in vendor/<name> against the
...
def cmd_refresh(name, subject=None, body="", amend=False):
    ...
    restamp(name)
    print(f"refresh: vendor/{name} left as-is and restamped; "
          f"verify with: python3 tools/vendor.py check {name}")
    return 0
...
def cmd_add(name, args):
    manifest_path = os.path.join(PROJECT, "vendor.toml")
    ...
    print(f"add: {name} pinned; selection in vendor-patches/{name}/files "
          ...
          f"vendor/{name}, then: tools/vendor.py refresh {name} -m \"...\"")
    return 0


USAGE = """\
tools/vendor.py                       assemble vendor/ (default; make vendor)
tools/vendor.py --force [CRATE]       reassemble even if stamped
tools/vendor.py diff CRATE            show working-tree delta vs the series
tools/vendor.py refresh CRATE -m "subject" [-b body]
                                      capture the delta as a NEW series patch
tools/vendor.py refresh CRATE --amend
                                      fold the delta into the LAST series patch
tools/vendor.py check CRATE           verify series reproduces the working tree
tools/vendor.py add CRATE --version V | --git URL --commit C [--subdir D]
                                      pin a new upstream (writes vendor.toml + files)
The everyday loop: make vendor → edit vendor/CRATE until green →
vendor.py refresh CRATE. Updating a pin: edit vendor.toml, make vendor,
re-spin any patch that fails to apply. Details: vendor-patches/README.md
"""
