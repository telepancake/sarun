...
pub enum Error {
    #[cfg(feature = "fetch")]
    // reqwest's Display intentionally collapses nested connector/TLS causes
    // to "error sending request".  Preserve its structured Debug cause so a
    // public acquisition failure identifies the actual transport boundary.
    #[error("http error: {0:?}")]
