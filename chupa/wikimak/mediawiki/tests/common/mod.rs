...
/// Render the relevant shape of a dumps.wikimedia.org directory listing.
/// Test filenames are controlled ASCII and need no HTML escaping.
pub fn build_directory_listing(files: &[(&str, u64)]) -> Vec<u8> {
    let mut out = String::from("<html><body><pre>\n");
    for (name, size) in files {
        out.push_str(&format!(
            "<a href=\"{name}\">{name}</a> 01-Jan-2026 00:00 {size}\n"
        ));
    }
