...
use common::{build_directory_listing, build_sha256sums, fixture};
...
fn discover_content_history_happy_path() {
    ...
    let listing = build_directory_listing(&[
        (p1.0, p1.1.len() as u64),
        (p2.0, p2.1.len() as u64),
        (p3.0, p3.1.len() as u64),
    ]);
    ...
    for part in &run.parts {
        ...
        let expected = [p1, p2, p3]
            .iter()
            .find(|(name, _)| *name == part.filename)
            .map(|(_, body)| body.len() as u64)
            .expect("part came from fixture manifest");
        assert_eq!(part.size_bytes, expected);
