...
#[test]
fn ru_module_arguments_wrapper_reads_localized_parent_frame() {
    let (mut store, _, _) = full_store("ru-82617");
    store.by_nsname.insert(
        (828, "Round".into()),
        r#"
        local arguments = require('Module:Arguments')
        local moduleRound = {}
        moduleRound.__index = moduleRound
        function moduleRound.new(frame)
            local self = setmetatable({}, moduleRound)
            self.args = arguments.getArgs(frame, { wrappers = { 'Template:Round' } })
            return self
        end
    ...
    for parent_title in ["Template:Round", "Шаблон:Round"] {
        let frame = Frame {
            args: BTreeMap::new(),
            parent: Some(Box::new(Frame {
                args: BTreeMap::from([("1".into(), "Россия".into())]),
                parent: None,
                // The input frame may carry either namespace spelling; the
                // Scribunto title/frame bridge exposes the canonical form.
                title: parent_title.into(),
            })),
            title: "Модуль:Round".into(),
        };
        let output = LuaInvoker::default()
            .invoke("Round", "main", &frame, &store)
            .expect("wrapper should select the parent arguments");
        assert_eq!(output, "Россия!");
    }
}

#[test]
fn ru_numeric_title_namespace_reads_template_content() {
    let (mut store, _, _) = full_store("ru-82617");
    store
        .by_nsname
        .insert((10, "Население/STA-00N".into()), "population template".into());
    store.by_nsname.insert(
        (828, "Statistical".into()),
        r#"
        local p = {}
        function p.main(frame)
    ...
    let output = LuaInvoker::default()
        .invoke("Statistical", "main", &frame, &store)
        .expect("numeric namespace title should read the template");
    assert_eq!(output, "10|population template");
}
