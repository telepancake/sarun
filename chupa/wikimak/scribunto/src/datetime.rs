...
/// L t U codes; unknown letters pass through. `lang` selects localized
/// variants whose meaning depends on the content language, such as `xg`.
pub fn format_php_date(fmt: &str, c: &Civil, unix: i64, lang: &str) -> String {
    let mut out = String::new();
    let mut chars = fmt.chars().peekable();
    while let Some(ch) = chars.next() {
        match ch {
            ...
            'x' if chars.peek() == Some(&'g') => {
                chars.next();
                out.push_str(month_name_genitive(lang, c.month));
            }
...
fn month_name_genitive(lang: &str, month: u32) -> &'static str {
    ...
    let language = lang.split(['-', '_']).next().unwrap_or(lang);
...
mod tests {
    ...
    fn php_date_codes() {
        ...
        assert_eq!(format_php_date("Y-m-d", &c, 0, "en"), "2005-03-01");
        assert_eq!(format_php_date("j F Y", &c, 0, "en"), "1 March 2005");
        assert_eq!(format_php_date("l", &c, 0, "en"), "Tuesday");
        assert_eq!(format_php_date("H:i:s", &c, 0, "en"), "12:34:56");
        assert_eq!(format_php_date("\\Y=Y", &c, 0, "en"), "Y=2005");
    }

    #[test]
    fn russian_genitive_month_token_is_one_format_code() {
        let c = civil_from_unix(unix_from_fields(2026, 10, 18, 0, 0, 0));
        assert_eq!(format_php_date("j xg Y", &c, 0, "ru"), "18 октября 2026");
        assert_eq!(format_php_date("j xg Y", &c, 0, "ru-RU"), "18 октября 2026");
    }
