...
/// Loaded as `return function(mw, host, frame) … end` and called once when a
/// page-scoped Lua session is initialized. `host` is a stable proxy whose
/// forwarded operations consult the current invocation's Rust host table.
...
function hash.hashValue(algo, value)
    ...
    if algo == "fnv164" then return host.fnv164(value) end
    error("mw.hash.hashValue: unsupported algorithm '" .. tostring(algo) .. "'")
end
function hash.listAlgorithms() return { "md5", "sha1", "fnv164" } end
...
mw.getCurrentFrame = function() return __sarun_current_frame end
...
pub fn builtin_lib(name: &str) -> Option<&'static str> {
    match name {
        ...
        // Lua 5.2 ships bit32 as a standard library and Wikimedia modules
        // require it directly.  The vendored Lua 5.1 runtime exposes the
        // compatible table installed by mwlib.
        "bit32" => Some("return bit32"),
...
/// Host operations that depend on the current PageStore/frame are forwarded
/// through this proxy. The proxy itself lives for the page render; the table
/// in `__sarun_current_host` is replaced for each (possibly nested) invoke.
pub const HOST_PROXY: &str = r#"
return function()
    local host = {}
    local function forward(name)
        return function(...)
            return __sarun_current_host[name](...)
        end
    end
    host.ustring_len = forward("ustring_len")
    host.ustring_sub = forward("ustring_sub")
    host.ustring_upper = forward("ustring_upper")
    host.ustring_lower = forward("ustring_lower")
    host.ustring_char = forward("ustring_char")
    host.ustring_codepoint = forward("ustring_codepoint")
    host.ustring_byteoffset = forward("ustring_byteoffset")
    host.cp_to_byte = forward("cp_to_byte")
    host.byte_to_cp = forward("byte_to_cp")
    host.byte = forward("byte")
    host.title_resolve = forward("title_resolve")
    host.title_make = forward("title_make")
    host.page_content = forward("page_content")
    host.message_plain = forward("message_plain")
    host.message_exists = forward("message_exists")
    host.format_date = forward("format_date")
    host.sha1 = forward("sha1")
    host.md5 = forward("md5")
    host.fnv164 = forward("fnv164")
    host.uri_encode = forward("uri_encode")
    host.uri_decode = forward("uri_decode")
    host.log = forward("log")
    host.current_title = forward("current_title")
    host.require = forward("require")
    host.site = __sarun_current_host.site
    host.lang_code = __sarun_current_host.lang_code
    host.rtl = __sarun_current_host.rtl
    return host
end
"#;
