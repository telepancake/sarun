...
Revision text MUST be decoded into a streaming zstd-backed representation;
neither one revision nor all revisions of one page may become a raw memory or
uncompressed temporary-file allocation. External sorting retains bounded keys
and metadata. Its run writer and merge reader stream record payloads, so a
merge head does not imply that its complete text is decoded.
...
64-way merge hierarchy and therefore at most `ceil(log_64 N)` merge levels.
...
Ordinary final frames are independent compression jobs submitted to an ordered
bounded pool. Results MAY complete out of order but MUST be written in record
order. An entity that outgrows ordinary raw-frame staging is one continuous
zstd frame streamed directly by the output owner; zstd's internal worker pool
compresses that frame without making the entity a retained memory unit. For a
frame with a reference prefix, the internal zstd job size is the actual prefix
length. Thus each repeated prefix initialization serves at least that much
input, rather than the former 1 MiB job repeatedly initializing a 16 MiB
prefix. A frame without a reference prefix leaves job sizing to zstd.

Peak final-compression memory is:

```text
O(K * normal_raw_frame + streaming_feed + zstd_worker_state(K, settings) + prefix)
```

There MUST NOT be an archive-sized or entity-sized raw buffer. A single page or
other entity may exceed the normal frame target, but its records are encoded
and compressed incrementally into one logical frame.

## 3. Frame and title directories

The frame directory is a fixed-width stream over final frame headers. Final
assembly already owns each completed header and compressed offset; it MUST
write the corresponding directory entry sequentially at that point. It MUST
NOT rediscover those entries by alternating 64-byte reads and payload seeks
over the completed archive. Validation or recovery without the sidecar may
reconstruct it by one large-buffered sequential scan that consumes payload
bytes, with constant working memory and at most one open data segment. Index
construction MUST consume the directory without creating a second
`Vec<FrameLocation>` proportional to `F`.

`ArchiveSetOutput` implements the normal write-time path. Tests compare its
fresh and crash-resumed output entry-for-entry with an independent archive
index; the direct assembly regression also asserts that fresh assembly makes
zero whole-set reconstruction calls.
...
queued frame state is bounded by `K` plus the current streaming-feed state.
...
| final compression + projection peak | bounded staging + configured zstd worker state + sorter workspace; independent of aggregate entity size |
...
pass counts and disk bounds do not change. An entity that exceeds ordinary
staging changes to direct streaming; its size does not become a memory or
uncompressed-temporary-space term.
