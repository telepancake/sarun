...
use wikimak_wikipedia::archive::{
    merge_many_archives_reusing_ref_prefix, ArchiveReader, CompressionSettings, EntityKind,
    Record,
};
...
#[test]
fn direct_archive_selects_whole_content_record_and_preserves_history_revisions() {
    let server = MockServer::start();
    let xml = fixture("export_three_pages.xml");
    let sha1_hex = hex::encode(Sha1::digest(&xml));
    ...
    // The fixture's exact readable content is page 2, revision 201, at this
    // timestamp. Build the same title index that makes the portable archive
    // user-readable, then exercise the public archive browse path rather than
    // inferring readability from the sequential record count below.
    let title_index = tmp.path().join("testwiki.swtitle");
    let generation_id =
        wikimak_wikipedia::generation::GenerationId::from_plan_bytes(b"sync-direct-readable");
    assert!(
        wikimak_wikipedia::title_index::build(&output, &title_index, &generation_id).unwrap() >= 3
    );
    let archive =
        wikimak_wikipedia::archive_browse::ArchiveBrowseIndex::open(&output, &title_index)
            .unwrap();
    let as_of = Utc
        .with_ymd_and_hms(2024, 1, 3, 0, 0, 0)
        .unwrap()
        .timestamp_micros();
    // A direct archive's PageState records the build observation time; the
    // revision timestamp remains the historical boundary for reading text.
    let page_id = archive.page_id_by_title("New Title", i64::MAX).unwrap();
    assert_eq!(page_id, 2);
    let expected_text = b"hello world, expanded.".to_vec();
    assert_eq!(archive.revision(page_id, 201).unwrap().unwrap().text, expected_text);
    assert_eq!(archive.page_text_at(page_id, as_of).unwrap(), Some(expected_text));
    ...
    let mut page1_revision_100 = None;
    let mut history_revision_states = Vec::new();
        ...
        while let Some(record) = frame.next_record().unwrap() {
            match record {
                Record::Revision { page_id, revision } => {
                    ...
                    if page_id == 1 && revision.meta.rev_id == 100 {
                        page1_revision_100 = Some((
                            revision.has_text,
                            revision.text.clone(),
                            revision.visibility.is_some(),
                            revision.history.is_some(),
                        ));
                    }
                    if revision.history.is_some() {
                        annotated_revisions += 1;
                        history_revision_states.push((
                            page_id,
                            revision.meta.rev_id,
                            revision.has_text,
                            revision.text.is_empty(),
                        ));
                    }
                }
    ...
    // Duplicate (page_id, revision_id) records are alternatives, not field
    // fragments: retain the complete text-bearing content record for 1/100.
    assert_eq!(
        page1_revision_100,
        Some((
            true,
            b"#REDIRECT [[New Title]]".to_vec(),
            false,
            false,
        ))
    );
    // The history parser represents the orphan revision without an upstream
    // page id as page 0; distinct history identities remain complete records.
    history_revision_states.sort_unstable();
    assert_eq!(
        history_revision_states,
        vec![(0, 102, false, true), (1, 101, false, true)]
    );
    assert_eq!(annotated_revisions, 2);
...
fn update_archive_overlaps_daily_runs_and_merges_with_full_archive() {
    ...
    .unwrap();
    // Updates are defined against a validated generation pair, not a bare
    // archive. Build the same generation-bound title/frame index that the
    // production installer publishes before asking for an update plan.
    let generation_id =
        wikimak_wikipedia::generation::GenerationId::from_plan_bytes(b"sync-update-base");
    assert_eq!(
        wikimak_wikipedia::cli_main(&[
            "title-index".into(),
            base.to_string_lossy().into_owned(),
            base.with_extension("swtitle")
                .to_string_lossy()
                .into_owned(),
            generation_id.as_str().into(),
        ]),
        0
    );
    ...
    merge_many_archives_reusing_ref_prefix(
        &base,
        &[base.clone(), update],
    ...
    let merged_generation_id =
        wikimak_wikipedia::generation::GenerationId::from_plan_bytes(b"sync-update-merged");
    assert_eq!(
        wikimak_wikipedia::cli_main(&[
            "title-index".into(),
            merged.to_string_lossy().into_owned(),
            merged
                .with_extension("swtitle")
                .to_string_lossy()
                .into_owned(),
            merged_generation_id.as_str().into(),
        ]),
        0
    );
