...
pub(crate) fn clear_run(root: &Path, plan: &DirectBuildPlan) -> Result<(), String> {
    initialize(root, plan)?;
    replace_header(&path(root), |header| {
        if header.plan_id == plan.plan_id {
            header.active_run_id = None;
        }
    })
}
...
pub(crate) fn source_writer(
    ...
    let plan = match crate::direct::read_direct_build_plan(&root.join("plan.json")) {
        Ok(plan) => plan,
        Err(_) => {
            let source = std::fs::read(root.join("source-plan.json"))
                .map_err(|error| error.to_string())?;
            let source: crate::direct::UpdateSourcePlan =
                serde_json::from_slice(&source).map_err(|error| error.to_string())?;
            crate::direct::validate_update_source_plan(&source)
                .map_err(|error| error.to_string())?;
            crate::direct::incremental_progress_plan(&source)
        }
    };
    source_writer_for_plan(root, &plan, target, part)
}

pub(crate) fn source_writer_for_plan(
    root: &Path,
    plan: &DirectBuildPlan,
    target: &str,
    part: &str,
    ...
    pub(crate) fn write(&self, value: &LiveTargetProgress) {
        ...
        let incoming = SourceValue {
            ...
            row: MirrorTargetProgress {
                ...
                fetch_server_timed_retries: value.fetch_server_timed_retries,
                fetch_robots_timed_retries: value.fetch_robots_timed_retries,
                fetch_fallback_timed_retries: value.fetch_fallback_timed_retries,
                fetch_local_spacing_timed_retries: value.fetch_local_spacing_timed_retries,
                bz2_admission_limit: value.bz2_admission_limit,
                bz2_admission_active_decoders: value.bz2_admission_active_decoders,
                bz2_admission_peak_active_decoders: value.bz2_admission_peak_active_decoders,
...
fn merge_source(previous: Option<SourceValue>, mut incoming: SourceValue) -> SourceValue {
    ...
    incoming.row.fetch_server_timed_retries = merge_network(
        previous.row.fetch_server_timed_retries,
        incoming.row.fetch_server_timed_retries,
    );
    incoming.row.fetch_robots_timed_retries = merge_network(
        previous.row.fetch_robots_timed_retries,
        incoming.row.fetch_robots_timed_retries,
    );
    incoming.row.fetch_fallback_timed_retries = merge_network(
        previous.row.fetch_fallback_timed_retries,
        incoming.row.fetch_fallback_timed_retries,
    );
    incoming.row.fetch_local_spacing_timed_retries = merge_network(
        previous.row.fetch_local_spacing_timed_retries,
        incoming.row.fetch_local_spacing_timed_retries,
    );
    let previous_bz2_sample = previous
        .updated_at_micros
        .max(previous.heartbeat_at_micros);
    let incoming_bz2_sample = incoming
        .updated_at_micros
        .max(incoming.heartbeat_at_micros);
    if !new_attempt && previous_bz2_sample > incoming_bz2_sample {
        incoming.row.bz2_admission_limit = previous.row.bz2_admission_limit;
        incoming.row.bz2_admission_active_decoders =
            previous.row.bz2_admission_active_decoders;
    }
    incoming.row.bz2_admission_peak_active_decoders = previous
        .row
        .bz2_admission_peak_active_decoders
        .max(incoming.row.bz2_admission_peak_active_decoders);
...
fn read_projection(
    ...
    if expected_run_id
        .is_some_and(|expected| header.active_run_id.as_deref() != Some(expected))
    {
        return Ok(None);
    }
    let expected_len = HEADER_BYTES
...
fn project(
    ...
    let mut fetch_server_timed_retries = 0_u64;
    let mut fetch_robots_timed_retries = 0_u64;
    let mut fetch_fallback_timed_retries = 0_u64;
    let mut fetch_local_spacing_timed_retries = 0_u64;
    let mut bz2_admission: Option<(u64, u64, u64, u64)> = None;
    for value in values.iter().flatten() {
        ...
        fetch_server_timed_retries = fetch_server_timed_retries
            .saturating_add(value.row.fetch_server_timed_retries);
        fetch_robots_timed_retries = fetch_robots_timed_retries
            .saturating_add(value.row.fetch_robots_timed_retries);
        fetch_fallback_timed_retries = fetch_fallback_timed_retries
            .saturating_add(value.row.fetch_fallback_timed_retries);
        fetch_local_spacing_timed_retries = fetch_local_spacing_timed_retries
            .saturating_add(value.row.fetch_local_spacing_timed_retries);
        if run_matches(value.run_id.as_deref()) {
            let sample_at = value.updated_at_micros.max(value.heartbeat_at_micros);
            if bz2_admission
                .as_ref()
                .map_or(true, |(previous_sample_at, ..)| sample_at >= *previous_sample_at)
            {
                bz2_admission = Some((
                    sample_at,
                    value.row.bz2_admission_limit,
                    value.row.bz2_admission_active_decoders,
                    value.row.bz2_admission_peak_active_decoders,
                ));
            }
        }
        if !run_matches(value.run_id.as_deref()) {
    ...
    let (_, bz2_admission_limit, bz2_admission_active_decoders, bz2_admission_peak_active_decoders) =
        bz2_admission.unwrap_or_default();
    MirrorBuildProgress {
        phase,
...
pub fn mirror_build_progress_for_run(
    ...
    let scratch = crate::cli::mirror_scratch_path(archive.as_ref());
    match crate::cli::active_update_progress_path(&scratch) {
        Ok(Some((update_id, progress))) => {
            read_projection(&progress, Some(&update_id), Some(run_id))
                .ok()
                .flatten()
        }
        Ok(None) => read_projection(&path(&scratch), None, Some(run_id)).ok().flatten(),
        Err(_) => None,
    }
}
...
mod tests {
    ...
    #[test]
    fn aggregate_retry_timing_and_latest_bz2_admission_snapshot() {
        let root = tempfile::tempdir().unwrap();
        let plan = plan();
        write_plan(root.path(), &plan);
        let left = source_writer(root.path(), "content-000000-source-000000", "a.xml").unwrap();
        let right = source_writer(root.path(), "content-000000-source-000001", "b.xml").unwrap();
        let mut left_value = live("content-000000-source-000000", "a.xml", 1, 40);
        left_value.updated_at_micros = 10;
        left_value.heartbeat_at_micros = 10;
        left_value.fetch_server_timed_retries = 2;
        left_value.fetch_robots_timed_retries = 3;
        left_value.fetch_fallback_timed_retries = 4;
        left_value.fetch_local_spacing_timed_retries = 5;
        left_value.bz2_admission_limit = 8;
        left_value.bz2_admission_active_decoders = 2;
        left_value.bz2_admission_peak_active_decoders = 4;
        let mut right_value = live("content-000000-source-000001", "b.xml", 2, 60);
        right_value.updated_at_micros = 20;
        right_value.heartbeat_at_micros = 20;
        right_value.fetch_server_timed_retries = 6;
        right_value.fetch_robots_timed_retries = 7;
        right_value.fetch_fallback_timed_retries = 8;
        right_value.fetch_local_spacing_timed_retries = 9;
        right_value.bz2_admission_limit = 12;
        right_value.bz2_admission_active_decoders = 3;
        right_value.bz2_admission_peak_active_decoders = 7;
        ...
        let progress = read_projection(&path(root.path()), Some(&plan.plan_id), None)
            .unwrap()
            .unwrap();
        assert_eq!(progress.fetch_server_timed_retries, 8);
        assert_eq!(progress.fetch_robots_timed_retries, 10);
        assert_eq!(progress.fetch_fallback_timed_retries, 12);
        assert_eq!(progress.fetch_local_spacing_timed_retries, 14);
        assert_eq!(progress.bz2_admission_limit, 12);
        assert_eq!(progress.bz2_admission_active_decoders, 3);
        assert_eq!(progress.bz2_admission_peak_active_decoders, 7);
    }
    ...
    fn retry_network_counters_and_logical_bytes_are_monotonic() {
        ...
        let mut first = live("content-000000-source-000000", "a.xml", 1, 80);
        first.fetch_server_timed_retries = 2;
        first.fetch_robots_timed_retries = 3;
        first.fetch_fallback_timed_retries = 4;
        first.fetch_local_spacing_timed_retries = 5;
        writer.write(&first);
        let mut second = live("content-000000-source-000000", "a.xml", 2, 20);
        second.fetch_server_timed_retries = 6;
        second.fetch_robots_timed_retries = 7;
        second.fetch_fallback_timed_retries = 8;
        second.fetch_local_spacing_timed_retries = 9;
        writer.write(&second);
        let progress = read_projection(&path(root.path()), Some(&plan.plan_id), None)
            .unwrap()
            .unwrap();
    ...
    #[test]
    fn aggregate_quiet_age_tracks_the_most_recent_active_target() {
        let root = tempfile::tempdir().unwrap();
        let plan = plan();
        write_plan(root.path(), &plan);
        let old =
            source_writer(root.path(), "content-000000-source-000000", "a.xml").unwrap();
        let fresh =
            source_writer(root.path(), "content-000000-source-000001", "b.xml").unwrap();
        let now = now_micros();
        let mut old_value = live("content-000000-source-000000", "a.xml", 1, 40);
        old_value.updated_at_micros = now.saturating_sub(120_000_000);
        let mut fresh_value = live("content-000000-source-000001", "b.xml", 1, 60);
        fresh_value.updated_at_micros = now.saturating_sub(2_000_000);
        old.write(&old_value);
        fresh.write(&fresh_value);
        ...
        assert!(progress
            .target_progress
            .iter()
            .any(|row| row.quiet_seconds >= 119));
        assert!(progress.active_quiet_seconds.is_some_and(|quiet| quiet <= 3));
    }
    ...
    #[test]
    fn incremental_projection_follows_active_selector_and_run_identity() {
        let root = tempfile::tempdir().unwrap();
        let destination = root.path().join("library/wiki.swdump");
        let scratch = crate::cli::mirror_scratch_path(&destination);
        let plan = plan();
        let update_root = scratch.join("updates").join(&plan.plan_id);
        std::fs::create_dir_all(&update_root).unwrap();
        initialize(&update_root, &plan).unwrap();
        begin_run(&update_root, &plan, "7").unwrap();
        std::fs::write(
            scratch.join("updates/active.json"),
            serde_json::to_vec(&serde_json::json!({
                "schema": 1,
                "update_id": plan.plan_id.clone(),
                "base_generation_id": "base"
            }))
            .unwrap(),
        )
        .unwrap();
        let writer = source_writer_for_plan(
            &update_root,
            &plan,
            "content-000000-source-000000",
            "a.xml",
        )
        .unwrap();
        writer.write(&live("content-000000-source-000000", "a.xml", 7, 40));

        let progress = mirror_build_progress_for_run(&destination, "7").unwrap();
        assert_eq!(progress.fetch_bytes_received, 40);
        assert_eq!(progress.target_progress.len(), 1);
        assert!(mirror_build_progress_for_run(&destination, "8").is_none());

        let foreign_root = scratch.join("updates/foreign");
        std::fs::create_dir_all(&foreign_root).unwrap();
        initialize(&foreign_root, &plan).unwrap();
        begin_run(&foreign_root, &plan, "7").unwrap();
        std::fs::write(
            scratch.join("updates/active.json"),
            serde_json::to_vec(&serde_json::json!({
                "schema": 1,
        ...
        .unwrap();
        assert!(mirror_build_progress_for_run(&destination, "7").is_none());
    }
}
