...
use crate::archive::{ArchiveError, EntityKey, EntityKind, FrameInfo, Result};
...
struct FrameDirectoryPart {
    name: String,
    entries: Vec<crate::frame_directory::FrameDirectoryEntry>,
}
...
enum WriteState {
    ...
    StreamingPayload {
        token: ArchiveSetStreamingFrameToken,
        written: u64,
    },
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct ArchiveSetStreamingFrameToken {
    header_offset: u64,
...
pub struct ArchiveSetOutput {
    ...
    frame_directory_entries: Option<Vec<crate::frame_directory::FrameDirectoryEntry>>,
    frame_directory_parts: Vec<FrameDirectoryPart>,
    frame_directory_writer: Option<crate::frame_directory::FrameDirectoryWriter>,
}
...
impl ArchiveSetOutput {
    pub fn new_in(parent: impl AsRef<Path>, range_target: u64) -> Result<Self> {
        ...
        Ok(Self {
            ...
            frame_directory_entries: None,
            frame_directory_parts: Vec::new(),
            frame_directory_writer: None,
        })
    }
    ...
    /// Stream one virtual directory beside the final archive while frames are
    /// produced. Fresh assembly therefore never reopens completed range data.
    /// On crash resume, only the already-sealed prefix lacks live writer
    /// metadata and is reconstructed by one buffered sequential pass before
    /// new output begins.
    pub(crate) fn write_frame_directory_to(
        mut self,
        path: impl AsRef<Path>,
        identity: [u8; 32],
    ) -> Result<Self> {
        let mut writer = crate::frame_directory::FrameDirectoryWriter::new(
            path.as_ref(),
            identity,
        )?;
        for segment in self.segments.iter().filter(|segment| segment.kind.is_some()) {
            crate::archive::visit_data_segment_frame_headers(
                self.root.path().join(&segment.name),
                |info, local_offset| {
                    writer.push(crate::frame_directory::FrameDirectoryEntry {
                        first_entity: info.first_entity,
                        last_entity: info.last_entity,
                        compressed_offset: segment
                            .virtual_start
        ...
        self.frame_directory_writer = Some(writer);
        Ok(self)
    }

    /// Retain exact data-frame metadata while this output is being written.
    ///
    /// This is deliberately opt-in: ordinary archive construction must not
    /// retain one directory entry per frame merely because the sink can
    /// observe frame headers. The incremental update path enables it for the
    /// one replacement piece whose directory is published from write-time
    /// evidence.
    pub(crate) fn collect_frame_directory_metadata(mut self) -> Self {
        self.frame_directory_entries = Some(Vec::new());
        self
    }
    ...
    fn seal_range(&mut self) -> Result<()> {
        ...
        self.seal_part(part, name.clone())?;
        if let Some(entries) = self.frame_directory_entries.take() {
            self.frame_directory_parts
                .push(FrameDirectoryPart { name, entries });
            self.frame_directory_entries = Some(Vec::new());
        }
        Ok(())
    }
    ...
    fn place_data_frame(
        &mut self,
        frame: FrameInfo,
        allow_provisional_values: bool,
    ) -> Result<FramePlacement> {
        validate_data_frame_info(frame, allow_provisional_values)?;
        let kind = frame.first_entity.kind;
        let first_id = frame.first_entity.id;
        let last_id = frame.last_entity.id;
        ...
        let header_offset = self
            .range
            .as_ref()
            .ok_or(ArchiveError::Invalid("archive frame has no range part"))?
            .bytes;
        let compressed_offset = header_offset
    ...
    fn collect_frame_metadata(
        ...
        let local_entry = crate::frame_directory::FrameDirectoryEntry {
            first_entity: frame.first_entity,
        ...
        if let Some(writer) = self.frame_directory_writer.as_mut() {
            let mut virtual_entry = local_entry;
            virtual_entry.compressed_offset = self
                .virtual_bytes
                .checked_add(compressed_offset)
                .ok_or(ArchiveError::FieldTooLarge)?;
            writer.push(virtual_entry)?;
        }
        Ok(self.frame_directory_entries.as_mut().map(|entries| {
    ...
    fn begin_frame(&mut self, header: &[u8]) -> Result<PayloadDestination> {
        ...
        let frame = parse_data_frame_header(header)?;
        let placement = self.place_data_frame(frame, false)?;
        self.write_range(header)?;
        self.collect_frame_metadata(frame, placement.compressed_offset)?;
    ...
    pub fn finish(mut self) -> Result<CompletedArchiveSet> {
        if !self.complete {
            return Err(ArchiveError::Invalid(
        ...
        if let Some(writer) = self.frame_directory_writer.take() {
            writer.finish()?;
        }
        Ok(CompletedArchiveSet {
            ...
            frame_directory_parts: self.frame_directory_parts,
        })
    }
}
    ...
    fn write(&mut self, mut input: &[u8]) -> std::io::Result<usize> {
        ...
        while !input.is_empty() {
            match &mut self.state {
                ...
                WriteState::StreamingPayload { token, written } => {
                    let token = *token;
                    let next_written = (*written)
                        ...
                        .ok_or_else(|| {
                            std::io::Error::new(
                                std::io::ErrorKind::InvalidData,
                                "streamed frame payload is too large",
                            )
                        })?;
                    self.write_range(input)?;
                    self.state = WriteState::StreamingPayload {
                        token,
...
impl crate::archive::StreamingFrameOutput for ArchiveSetOutput {
    type StreamingFrameToken = ArchiveSetStreamingFrameToken;
    ...
    fn begin_streaming_frame(
        ...
        if !matches!(self.state, WriteState::FrameHeader(ref bytes) if bytes.is_empty()) {
            return Err(ArchiveError::Invalid(
                "streamed frame must begin at an empty frame header",
            ));
        }
        provisional.compressed_bytes = 0;
        let placement = self.place_data_frame(provisional, true)?;
        let mut header = Vec::with_capacity(FRAME_HEADER_BYTES);
        crate::archive::write_frame_header(&mut header, provisional)?;
        self.write_range(&header)?;
        let directory_index = self.frame_directory_entries.as_mut().map(|entries| {
            let index = entries.len();
            entries.push(crate::frame_directory::FrameDirectoryEntry {
                first_entity: provisional.first_entity,
                last_entity: provisional.last_entity,
                compressed_offset: placement.compressed_offset,
                records: provisional.records,
                raw_bytes: provisional.raw_bytes,
                compressed_bytes: provisional.compressed_bytes,
                dictionary_id: provisional.dictionary_id,
            });
            index
        });
        let token = ArchiveSetStreamingFrameToken {
            header_offset: placement.header_offset,
            provisional,
            directory_index,
        };
        self.state = WriteState::StreamingPayload { token, written: 0 };
        Ok(token)
    }
    ...
    fn finish_streaming_frame(
        ...
        let (active, written) = match &self.state {
            WriteState::StreamingPayload { token, written } => (*token, *written),
            _ => {
                return Err(ArchiveError::Invalid(
                    "streamed frame has no active payload",
                ));
            }
        };
        if active != token {
            return Err(ArchiveError::Invalid(
                "streamed frame completion token does not match the active payload",
            ));
        }
        validate_data_frame_info(completed, false)?;
        if completed.first_entity != active.provisional.first_entity
            || completed.dictionary_id != active.provisional.dictionary_id
        {
            return Err(ArchiveError::Invalid(
                "streamed frame identity changed while compressed",
            ));
        }
        if written != completed.compressed_bytes {
            return Err(ArchiveError::Invalid(
        ...
        let part = self
            .range
            .as_mut()
            .ok_or(ArchiveError::Invalid("streamed frame range is absent"))?;
        if part.bytes != end || part.file.stream_position()? != end {
            return Err(ArchiveError::Invalid(
                "streamed frame range end changed while compressing",
            ));
        }
        part.file.seek(SeekFrom::Start(active.header_offset))?;
        crate::archive::write_frame_header(&mut part.file, completed)?;
        part.file.seek(SeekFrom::Start(end))?;
        part.last_id = completed.last_entity.id;
        if let Some(index) = active.directory_index {
            let entries = self.frame_directory_entries.as_mut().ok_or(
                ArchiveError::Invalid("streamed frame metadata collection ended early"),
            )?;
            let entry = entries.get_mut(index).ok_or(ArchiveError::Invalid(
                "streamed frame metadata entry is absent",
            ))?;
            *entry = crate::frame_directory::FrameDirectoryEntry {
                first_entity: completed.first_entity,
                last_entity: completed.last_entity,
                compressed_offset: active
                    .header_offset
                    .checked_add(FRAME_HEADER_BYTES as u64)
                    .ok_or(ArchiveError::FieldTooLarge)?,
                records: completed.records,
                raw_bytes: completed.raw_bytes,
                compressed_bytes: completed.compressed_bytes,
                dictionary_id: completed.dictionary_id,
            };
        }
        if let Some(writer) = self.frame_directory_writer.as_mut() {
            writer.push(crate::frame_directory::FrameDirectoryEntry {
                first_entity: completed.first_entity,
                last_entity: completed.last_entity,
                compressed_offset: self
                    .virtual_bytes
                    .checked_add(active.header_offset)
                    .and_then(|offset| offset.checked_add(FRAME_HEADER_BYTES as u64))
                    .ok_or(ArchiveError::FieldTooLarge)?,
                records: completed.records,
                raw_bytes: completed.raw_bytes,
                compressed_bytes: completed.compressed_bytes,
        ...
        self.state = WriteState::FrameHeader(Vec::with_capacity(FRAME_HEADER_BYTES));
        Ok(())
    }
}
...
fn parse_data_frame_header(
    ...
    if header.len() != FRAME_HEADER_BYTES
        ...
        || u32::from_le_bytes(header[4..8].try_into().expect("frame header length bytes"))
            as usize
            != FRAME_HEADER_BYTES
        || header[10..16].iter().any(|byte| *byte != 0)
        || header[60..64].iter().any(|byte| *byte != 0)
    {
        return Err(ArchiveError::Invalid("malformed archive frame header"));
    }
    let first_entity = EntityKey {
        kind: EntityKind::try_from(header[8])?,
        id: u64::from_le_bytes(header[16..24].try_into().expect("first entity ID bytes")),
    };
    let last_entity = EntityKey {
        kind: EntityKind::try_from(header[9])?,
        id: u64::from_le_bytes(header[24..32].try_into().expect("last entity ID bytes")),
    };
    let records = u64::from_le_bytes(header[32..40].try_into().expect("record count bytes"));
    let raw_bytes = u64::from_le_bytes(header[40..48].try_into().expect("raw byte count bytes"));
    let compressed_bytes =
        u64::from_le_bytes(header[48..56].try_into().expect("compressed byte count bytes"));
    let dictionary_id = match u32::from_le_bytes(
        header[56..60]
            .try_into()
            .expect("dictionary ID bytes"),
    ) {
        0 => None,
        id => Some(id),
    };
    ...
    validate_data_frame_info(frame, false)?;
    Ok(frame)
}

fn validate_data_frame_info(frame: FrameInfo, allow_provisional_values: bool) -> Result<()> {
    if frame.first_entity.kind != frame.last_entity.kind
        || frame.first_entity > frame.last_entity
        || (!allow_provisional_values
            && (frame.records == 0 || frame.compressed_bytes == 0))
        || frame.dictionary_id == Some(0)
    {
        return Err(ArchiveError::Invalid("invalid archive frame header"));
    }
    Ok(())
}
...
impl CompletedArchiveSet {
    pub(crate) fn frame_directory_entries_for(
        &self,
        segment_name: &str,
    ) -> Result<&[crate::frame_directory::FrameDirectoryEntry]> {
        self.frame_directory_parts
            .iter()
            .find(|part| part.name == segment_name)
            .map(|part| part.entries.as_slice())
            .ok_or(ArchiveError::Invalid(
                "write-time frame metadata was not collected for archive segment",
            ))
    }
}
...
pub struct ArchiveSetReader {
    root: PathBuf,
    _lease: Option<crate::archive::ArchiveSharedLease>,
    segments: Vec<ArchiveSetSegment>,
    position: u64,
    length: u64,
...
impl ArchiveSetReader {
    ...
    /// Open the exact archive-set members while leaving unrelated direct
    /// children outside the logical stream. This is used only to migrate an
    /// old generation into a digest-bound ownership manifest; callers must
    /// consume and validate the complete archive before treating the selected
    /// members as owned.
    pub(crate) fn open_with_residue(root: impl AsRef<Path>) -> Result<Self> {
        Self::open_impl(root.as_ref(), true, true)
    }

    /// Open while the caller proves it already owns this generation's
    /// exclusive cleanup lease. This exists only for semantic validation of a
    /// legacy generation immediately before receipted retirement.
    pub(crate) fn open_with_residue_under_cleanup_lease(
        root: impl AsRef<Path>,
    ...
    fn open_impl(root: &Path, allow_residue: bool, acquire_lease: bool) -> Result<Self> {
        ...
        let lease = if acquire_lease {
            Some(
                crate::archive::try_acquire_archive_shared_lease(&root)?.ok_or_else(|| {
        ...
        for name in names {
            let name = match name.into_string() {
                Ok(name) => name,
                Err(_) if allow_residue => continue,
                Err(_) => {
                    return Err(ArchiveError::Invalid("archive-set filename is not UTF-8"));
                }
            };
            if !name.ends_with(PART_SUFFIX) {
                if allow_residue {
            ...
            let (kind, first_id, last_id) = match parse_segment_name(&name) {
                Ok(parsed) => parsed,
                Err(_) if allow_residue => continue,
...
mod tests {
    ...
    use super::*;
    use crate::archive::{
        ArchiveReader, ArchiveWriter, CompressionSettings, Record, StreamingFrameOutput,
    };
    ...
    fn physical_parts_are_one_logical_archive() {
        ...
        let directory_path = parent.path().join("testwiki.swframe");
        let identity = [19_u8; 32];
        let output = ArchiveSetOutput::new_in(parent.path(), 1024)
            .unwrap()
            .write_frame_directory_to(&directory_path, identity)
            .unwrap();
        ...
        .unwrap();
        let (_, locations, complete) = crate::archive::index_file(&destination).unwrap();
        assert!(complete);
        assert_eq!(directory.len(), locations.len());
        for (entry, location) in directory.iter().zip(&locations) {
            assert_eq!(
                entry.unwrap(),
                crate::frame_directory::FrameDirectoryEntry::from(location)
            );
        }
    ...
    fn resumable_output_discards_only_the_open_range() {
        ...
        let output = ArchiveSetOutput::resumable_in(parent.path(), name, 1024)
            .unwrap()
            .write_frame_directory_to(&directory_path, identity)
            .unwrap();
        let boundary = output
        ...
        let mut reader =
            ArchiveReader::new(ArchiveSetReader::open(&destination).unwrap()).unwrap();
        let mut decoded = Vec::new();
        while let Some(mut frame) = reader.next_frame().unwrap() {
            while let Some(record) = frame.next_record().unwrap() {
                decoded.push(record);
    ...
    #[test]
    fn streaming_frame_patches_current_range_and_metadata_without_sealing() {
        let parent = tempfile::tempdir().unwrap();
        let mut output = ArchiveSetOutput::new_in(parent.path(), 1)
            .unwrap()
            .collect_frame_directory_metadata();
        let mut file_header = [0_u8; FILE_HEADER_BYTES];
        file_header[..FILE_MAGIC.len()].copy_from_slice(FILE_MAGIC);
        output.write_all(&file_header).unwrap();

        let provisional = FrameInfo {
        ...
        let token = output.begin_streaming_frame(provisional).unwrap();
        let segments_before_payload = output.segments().len();
        let payload = b"streamed";
        output.write_all(payload).unwrap();
        let completed = FrameInfo {
            first_entity: provisional.first_entity,
            last_entity: EntityKey {
                kind: EntityKind::Page,
                id: 2,
        ...
        output.finish_streaming_frame(token, completed).unwrap();
        assert_eq!(output.segments().len(), segments_before_payload);
        assert_eq!(output.range.as_ref().unwrap().bytes, 64 + payload.len() as u64);
        assert_eq!(output.frame_directory_entries.as_ref().unwrap()[0].frame_info(), completed);

        let mut done = [0_u8; FRAME_HEADER_BYTES];
        done[..DONE_MAGIC.len()].copy_from_slice(DONE_MAGIC);
        output.write_all(&done).unwrap();
        let completed_set = output.finish().unwrap();
        let range_name = completed_set
            .segments
            .iter()
            ...
            .clone();
        assert_eq!(
            completed_set.frame_directory_entries_for(&range_name)
                .unwrap(),
            &[crate::frame_directory::FrameDirectoryEntry {
                first_entity: completed.first_entity,
                last_entity: completed.last_entity,
                compressed_offset: FRAME_HEADER_BYTES as u64,
                records: completed.records,
                raw_bytes: completed.raw_bytes,
                compressed_bytes: completed.compressed_bytes,
                dictionary_id: completed.dictionary_id,
        ...
        let destination = parent.path().join("streamed.swdump");
        completed_set.persist(&destination).unwrap();
        let range = std::fs::read(destination.join(range_name)).unwrap();
        assert_eq!(
            u64::from_le_bytes(range[48..56].try_into().unwrap()),
            payload.len() as u64
        );
        assert_eq!(&range[FRAME_HEADER_BYTES..], payload);
    }
}
