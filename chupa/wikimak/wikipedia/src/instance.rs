...
impl Instance {
    ...
    pub fn open(cfg: InstanceConfig) -> Result<Self> {
        ...
        ensure_title_pool_receipt(&cfg.root, title_generation, title_shard_count, &titles_dir)?;
        if conn
    ...
    pub fn flush(&self) -> Result<()> {
        ...
        let generation = persisted_title_pool_generation(&g.conn)?;
        refresh_title_pool_receipt(
            &self.root,
            generation,
            self.title_shard_count.load(Ordering::Relaxed),
            &title_pool_dir(&self.root, generation),
        )?;
    ...
    fn reshard_titles_once(&self, g: &mut InstanceInner) -> Result<()> {
        ...
        let old_generation = persisted_title_pool_generation(&g.conn)?;
        refresh_title_pool_receipt(
            &self.root,
            old_generation,
            old_count,
            &title_pool_dir(&self.root, old_generation),
        )?;
        ...
        ensure_title_pool_receipt(&self.root, generation, new_count, &new_dir)?;
...
fn title_pool_generation_is_real_dir(
    root: &std::path::Path,
    generation: u32,
) -> Result<bool> {
    match std::fs::symlink_metadata(title_pool_dir(root, generation)) {
...
const TITLE_POOL_RECEIPT_SCHEMA: u32 = 2;
const MAX_TITLE_POOL_RECEIPT_SHARDS: u32 = 1_000_000;
const TITLE_POOL_QUARANTINE_ATTEMPTS: u32 = 1024;

#[derive(Debug, Deserialize, Serialize)]
struct TitlePoolReceipt {
    schema: u32,
    generation: u32,
    shard_count: u32,
    shards: Vec<TitlePoolShardReceipt>,
}

#[derive(Debug, Deserialize, Serialize)]
struct TitlePoolShardReceipt {
    name: String,
    bytes: u64,
    identity: TitlePoolFileIdentity,
    digest: String,
}

#[derive(Debug, Deserialize, Serialize, Eq, PartialEq)]
struct TitlePoolFileIdentity {
    device: u64,
    inode: u64,
    birth_seconds: i64,
    birth_nanos: i64,
...
fn title_pool_receipt_path(root: &std::path::Path, generation: u32) -> PathBuf {
    if generation == 0 {
...
fn title_pool_valid_digest(value: &str) -> bool {
    value.len() == 64
        && value
            .bytes()
...
fn title_pool_file_identity(
    file: &std::fs::File,
    path: &std::path::Path,
) -> Result<TitlePoolFileIdentity> {
    let _ = path;
    let metadata = file.metadata()?;
    if !metadata.file_type().is_file() {
        return Err(Error::Corrupt("title pool shard is not a regular file"));
    }
    let birth = metadata
        .created()
        .map_err(Error::Io)?
        .duration_since(std::time::UNIX_EPOCH)
        .map_err(|_| Error::Corrupt("title pool birth time is before epoch"))?;
    let modified = metadata
        .modified()
        .map_err(Error::Io)?
        .duration_since(std::time::UNIX_EPOCH)
        .map_err(|_| Error::Corrupt("title pool modification time is before epoch"))?;
    #[cfg(unix)]
    {
        use std::os::unix::fs::MetadataExt;
        return Ok(TitlePoolFileIdentity {
            device: metadata.dev(),
            inode: metadata.ino(),
            birth_seconds: birth.as_secs() as i64,
            birth_nanos: birth.subsec_nanos() as i64,
    ...
    #[cfg(not(unix))]
    {
        let _ = (metadata, birth, modified);
        Err(Error::Corrupt("title pool file identity is unsupported on this platform"))
    }
}

fn open_title_pool_file(
    path: &std::path::Path,
) -> Result<(std::fs::File, TitlePoolFileIdentity)> {
    #[cfg(not(unix))]
    return Err(Error::Corrupt("title pool no-follow is unsupported on this platform"));
    ...
    let identity = title_pool_file_identity(&file, path)?;
    Ok((file, identity))
}

fn inspect_title_pool_file_identity(path: &std::path::Path) -> Result<TitlePoolFileIdentity> {
    open_title_pool_file(path).map(|(_, identity)| identity)
}

fn digest_title_pool_file(
    path: &std::path::Path,
    expected_bytes: Option<u64>,
) -> Result<(TitlePoolFileIdentity, String)> {
    let (mut file, identity) = open_title_pool_file(path)?;
    if expected_bytes.is_some_and(|expected| expected != identity.bytes) {
        return Err(Error::Corrupt("title pool shard length changed"));
    }
    ...
    let final_identity = title_pool_file_identity(&file, path)?;
    if final_identity != identity
        || expected_bytes.is_some_and(|expected| expected != final_identity.bytes)
    {
        return Err(Error::Corrupt("title pool shard changed while hashing"));
    }
    Ok((final_identity, hex::encode(hasher.finalize())))
}

fn validate_title_pool_receipt(
    receipt: &TitlePoolReceipt,
    generation: u32,
) -> Result<()> {
    if receipt.schema != TITLE_POOL_RECEIPT_SCHEMA
        || receipt.generation != generation
        || receipt.shard_count == 0
        || receipt.shard_count > MAX_TITLE_POOL_RECEIPT_SHARDS
        || receipt.shards.len() > receipt.shard_count as usize
    {
        return Err(Error::Corrupt("title pool ownership receipt"));
    }
    let mut previous = None;
    for shard in &receipt.shards {
        let Some(raw_id) = shard.name.strip_prefix("shard-") else {
            return Err(Error::Corrupt("title pool ownership receipt shard name"));
        };
        let Ok(id) = raw_id.parse::<u32>() else {
            return Err(Error::Corrupt("title pool ownership receipt shard name"));
        };
        if format!("shard-{id:04}") != shard.name
            || id >= receipt.shard_count
            || shard.bytes == 0
            || !title_pool_valid_digest(&shard.digest)
            || previous.is_some_and(|name: &str| name >= shard.name.as_str())
        {
            return Err(Error::Corrupt("title pool ownership receipt shard"));
        }
        previous = Some(shard.name.as_str());
    }
    Ok(())
}

fn read_title_pool_receipt(
    root: &std::path::Path,
    generation: u32,
) -> Result<Option<TitlePoolReceipt>> {
    let path = title_pool_receipt_path(root, generation);
    match std::fs::symlink_metadata(&path) {
        Ok(metadata) if metadata.file_type().is_file() => {}
        Ok(_) => return Err(Error::Corrupt("title pool ownership receipt is not a file")),
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => return Ok(None),
        Err(error) => return Err(Error::Io(error)),
    }
    let (mut file, _) = open_title_pool_file(&path)?;
    let mut bytes = Vec::new();
    file.read_to_end(&mut bytes)?;
    let receipt: TitlePoolReceipt =
        serde_json::from_slice(&bytes).map_err(|_| Error::Corrupt("title pool ownership receipt"))?;
    validate_title_pool_receipt(&receipt, generation)?;
    Ok(Some(receipt))
}

fn persist_title_pool_receipt(
    root: &std::path::Path,
    receipt: &TitlePoolReceipt,
) -> Result<()> {
    validate_title_pool_receipt(receipt, receipt.generation)?;
    let path = title_pool_receipt_path(root, receipt.generation);
    if let Ok(metadata) = std::fs::symlink_metadata(&path) {
        if !metadata.file_type().is_file() {
            return Err(Error::Corrupt("title pool ownership receipt is not a file"));
        }
    }
    let mut temporary = tempfile::NamedTempFile::new_in(root)?;
    serde_json::to_writer(temporary.as_file_mut(), receipt)
        .map_err(|_| Error::Corrupt("encode title pool ownership receipt"))?;
...
fn capture_title_pool_receipt(
    generation: u32,
    shard_count: u32,
    titles_dir: &std::path::Path,
    previous: Option<&TitlePoolReceipt>,
) -> Result<TitlePoolReceipt> {
    if shard_count == 0 || shard_count > MAX_TITLE_POOL_RECEIPT_SHARDS {
        return Err(Error::Corrupt("title pool ownership receipt shard bound"));
    }
    let mut shards = Vec::new();
    for shard_id in 0..shard_count {
        let path = titles_dir.join(format!("shard-{shard_id:04}"));
        let identity = match inspect_title_pool_file_identity(&path) {
            Ok(identity) => identity,
            Err(Error::Io(error)) if error.kind() == std::io::ErrorKind::NotFound => continue,
            Err(error) => return Err(error),
        };
        let name = format!("shard-{shard_id:04}");
        let digest = match previous
            .and_then(|receipt| receipt.shards.iter().find(|shard| shard.name == name))
            .filter(|shard| shard.bytes == identity.bytes && shard.identity == identity)
        {
            Some(shard) => shard.digest.clone(),
            None => digest_title_pool_file(&path, Some(identity.bytes))?.1,
        };
        shards.push(TitlePoolShardReceipt {
            name,
    ...
    let receipt = TitlePoolReceipt {
        schema: TITLE_POOL_RECEIPT_SCHEMA,
        generation,
        shard_count,
        shards,
    };
...
fn validate_title_pool_files(
    receipt: &TitlePoolReceipt,
    titles_dir: &std::path::Path,
) -> Result<()> {
    for shard in &receipt.shards {
        let identity = inspect_title_pool_file_identity(&titles_dir.join(&shard.name))?;
        if identity != shard.identity || identity.bytes != shard.bytes {
            return Err(Error::Corrupt("title pool shard identity changed"));
        }
    }
    Ok(())
}

fn ensure_title_pool_receipt(
    root: &std::path::Path,
    generation: u32,
    shard_count: u32,
    titles_dir: &std::path::Path,
    ...
    if let Some(receipt) = read_title_pool_receipt(root, generation)? {
        if receipt.shard_count != shard_count {
            return Err(Error::Corrupt("title pool ownership receipt shard count"));
        }
        validate_title_pool_files(&receipt, titles_dir)?;
        return Ok(());
    }
    let receipt = capture_title_pool_receipt(generation, shard_count, titles_dir, None)?;
    persist_title_pool_receipt(root, &receipt)
}

fn refresh_title_pool_receipt(
    root: &std::path::Path,
    generation: u32,
    shard_count: u32,
    titles_dir: &std::path::Path,
) -> Result<()> {
    let previous = read_title_pool_receipt(root, generation)?;
    let receipt = capture_title_pool_receipt(
        generation,
        shard_count,
        titles_dir,
        previous.as_ref(),
...
pub(crate) fn rename_without_replacing(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let destination = CString::new(destination.as_os_str().as_bytes())
            .map_err(|_| std::io::Error::new(std::io::ErrorKind::InvalidInput, "destination contains NUL"))?;
        let rc = unsafe {
            libc::renameatx_np(
                libc::AT_FDCWD,
                source.as_ptr(),
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let destination = CString::new(destination.as_os_str().as_bytes())
            .map_err(|_| std::io::Error::new(std::io::ErrorKind::InvalidInput, "destination contains NUL"))?;
        // musl's libc crate target exposes the syscall number but not the
        // renameat2 wrapper. Calling the kernel entry point keeps the atomic
        // no-replace contract identical across glibc and musl builds.
        let rc = unsafe {
            libc::syscall(
                libc::SYS_renameat2,
                libc::AT_FDCWD,
...
fn quarantine_title_entry(
    root: &std::path::Path,
    entry: &std::path::Path,
    label: &str,
) -> Result<PathBuf> {
    let quarantine = title_pool_quarantine_dir(root)?;
    let name = entry
        .file_name()
        .ok_or(Error::Corrupt("title pool quarantine entry name"))?
        .to_string_lossy();
    for counter in 0..TITLE_POOL_QUARANTINE_ATTEMPTS {
        let destination = quarantine.join(format!(
            "{label}-{name}-{}-{counter}",
            std::process::id()
        ));
        match rename_without_replacing(entry, &destination) {
            Ok(()) => {
                sync_parent_dir(entry);
    ...
    Err(Error::Corrupt("title pool quarantine namespace exhausted"))
}

fn title_pool_quarantine_dir(root: &std::path::Path) -> Result<PathBuf> {
    let quarantine = root.join(".title-pool-quarantine");
    match std::fs::symlink_metadata(&quarantine) {
        Ok(metadata) if metadata.file_type().is_dir() => {}
        Ok(_) => return Err(Error::Corrupt("title pool quarantine namespace")),
...
fn retire_receipted_title_generation(
    root: &std::path::Path,
    generation: u32,
    ...
    let quarantine = title_pool_quarantine_dir(root)?;
    for shard in &receipt.shards {
        let path = generation_dir.join(&shard.name);
        let claim = quarantine.join(format!("title-owned-g{generation}-{}", shard.name));
        let claimed = match std::fs::symlink_metadata(&claim) {
            Ok(_) => Some(claim),
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
                match rename_without_replacing(&path, &claim) {
                    Ok(()) => {
                        sync_parent_dir(&path);
                        sync_parent_dir(&claim);
                        Some(claim)
                    }
                    Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => Some(claim),
        ...
        match digest_title_pool_file(&claimed, Some(shard.bytes)) {
            Ok((_, digest)) if digest == shard.digest => {
                // The no-replace claim makes the validated candidate
                // independent of the original shard pathname. A hostile actor
                // with access to the private quarantine namespace could still
                // mutate the claimed path after hashing and before unlink;
                // the instance writer lock excludes Sarun writers, and this
                // residual filesystem race is documented rather than hidden.
                std::fs::remove_file(&claimed)?;
            }
            Ok((_, digest)) => eprintln!(
                "title shard {} digest mismatch ({digest}); retained at {}",
                shard.name,
                claimed.display()
            ),
            Err(error) => eprintln!(
                "title shard {} failed ownership validation ({error}); retained at {}",
                shard.name,
                claimed.display()
            ),
        }
    ...
    let residual = std::fs::read_dir(generation_dir)?.collect::<std::io::Result<Vec<_>>>()?;
    for entry in residual {
        quarantine_title_entry(root, &entry.path(), &format!("title-generation-g{generation}"))?;
    }
    std::fs::remove_dir(generation_dir)?;
    sync_parent_dir(generation_dir);

    let receipt_path = title_pool_receipt_path(root, generation);
    match std::fs::symlink_metadata(&receipt_path) {
        Ok(metadata) if metadata.file_type().is_file() => std::fs::remove_file(&receipt_path)?,
        Ok(_) => return Err(Error::Corrupt("title pool ownership receipt is not a file")),
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {}
        Err(error) => return Err(Error::Io(error)),
    }
    sync_parent_dir(&receipt_path);
    Ok(())
}

/// Retire recognized, unselected immutable title-pool generations without
/// recursively deleting anything. A receipt permits unlinking only the
/// exact shard files it owns; an unreceipted generation is moved as one
/// namespace entry to quarantine, preserving all nested data.
fn gc_stale_title_generations(root: &std::path::Path, selected: u32) -> Result<()> {
    for entry in std::fs::read_dir(root)? {
        ...
        if generation != selected {
            let path = entry.path();
            match read_title_pool_receipt(root, generation)? {
                Some(receipt) => retire_receipted_title_generation(root, generation, &path, &receipt)?,
                None => {
                    let retained = quarantine_title_entry(root, &path, "title-generation")?;
                    eprintln!(
                        "title generation {generation} lacks an ownership receipt; retained at {}",
                        retained.display()
                    );
                }
            }
    ...
    // A crash may have removed a retired generation after its receipt was
    // persisted but before the receipt unlink. Remove only a valid, regular
    // receipt whose generation is absent; this is the idempotent completion
    // cut for the sidecar protocol.
    ...
    for entry in entries {
        ...
        let generation = if name == ".titles.receipt" {
            Some(0)
        } else {
            name.strip_prefix(".titles-g")
                .and_then(|suffix| suffix.strip_suffix(".receipt"))
                .and_then(|suffix| suffix.parse::<u32>().ok())
        };
        let Some(generation) = generation else {
            continue;
        };
        if generation == selected || title_pool_generation_is_real_dir(root, generation)? {
            continue;
        }
        let receipt_path = title_pool_receipt_path(root, generation);
        if read_title_pool_receipt(root, generation)?.is_some() {
            match std::fs::symlink_metadata(&receipt_path) {
                Ok(metadata) if metadata.file_type().is_file() => {
