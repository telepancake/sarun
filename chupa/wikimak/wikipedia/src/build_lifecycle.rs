...
pub(crate) struct TargetReceipt {
    ...
    /// Exact immutable inventory of every published target file except this
    /// receipt. Cleanup may unlink a claimed file only when this identity
    /// still matches immediately after the claim.
    #[serde(default)]
...
pub(crate) struct TargetCheckpointFile {
    pub(crate) name: String,
    pub(crate) bytes: u64,
    /// On Unix this is a device/inode/mtime tuple. It is captured from the
    /// already-written file metadata and detects same-name/same-size
    /// replacement without another full content pass. Unsupported platforms
    /// leave it absent and therefore fail closed at cleanup.
    #[serde(default)]
...
pub(crate) fn commit_title_projection(
    ...
    let bytes = std::fs::metadata(&path)
        ...
        .len();
    if crate::title_projection::legacy_projection_file_bytes(entries) == Some(bytes) {
        return Err(invalid(
    ...
    if crate::title_projection::projection_file_bytes(entries) != Some(bytes) {
        return Err(invalid(
            InvalidBuildKind::CorruptArtifact,
            &path,
    ...
    let receipt = TitleProjectionReceipt {
        ...
        format_version: crate::title_projection::PROJECTION_FILE_VERSION,
...
fn inspect_title_projection(
    ...
    if receipt.schema != RECEIPT_SCHEMA
        || receipt.format_version != crate::title_projection::PROJECTION_FILE_VERSION
        ...
        || receipt.generation_id != GenerationId::from_plan_id(&plan.plan_id)
    {
        return Err(invalid(
            ...
            &receipt_path,
            "title projection receipt is obsolete or belongs to another plan; rebuild required",
        ));
    }
    ...
    let bytes = std::fs::metadata(&path)
        ...
        .len();
    if crate::title_projection::legacy_projection_file_bytes(receipt.entries) == Some(bytes) {
        return Err(invalid(
            InvalidBuildKind::UnsupportedSchema,
            &path,
            "title projection uses the legacy 16-byte format; rebuild required",
    ...
    if bytes != receipt.bytes
        || crate::title_projection::projection_file_bytes(receipt.entries) != Some(bytes)
    {
        return Err(invalid(
            InvalidBuildKind::CorruptArtifact,
            &path,
...
fn quarantine_uncommitted_entry(
    root: &Path,
    path: &Path,
    label: &str,
    ...
    match std::fs::symlink_metadata(&quarantine) {
        ...
        Ok(_) => {
            return Err(invalid(
                ...
                &quarantine,
                "cleanup quarantine is not a real directory",
            ))
        }
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
            std::fs::create_dir(&quarantine).map_err(|error| {
                invalid(
                    InvalidBuildKind::Io,
                    &quarantine,
                    format!("cannot create cleanup quarantine: {error}"),
                )
            })?;
        }
        Err(error) => return Err(invalid(InvalidBuildKind::Io, &quarantine, error.to_string())),
    }
    let name = path
        .file_name()
        .ok_or_else(|| invalid(InvalidBuildKind::UnsupportedLayout, path, "entry has no name"))?
        .to_string_lossy();
    for counter in 0_u32..1024 {
        let destination = quarantine.join(format!("{label}-{name}-{counter}"));
        match crate::instance::rename_without_replacing(path, &destination) {
            Ok(()) => {
                sync_directory(root)?;
            ...
            Err(error) => return Err(invalid(InvalidBuildKind::Io, path, error.to_string())),
        }
    }
    Err(invalid(
        InvalidBuildKind::Io,
        path,
        "cleanup quarantine namespace exhausted",
    ))
}
...
fn identify_regular_file(path: &Path) -> std::io::Result<(u64, Option<String>)> {
...
fn expected_target_files(kind: TargetKind, index: usize) -> Vec<&'static str> {
    let mut files = vec!["data.swdump", "data.swframe", "title-records.swdump"];
    if kind == TargetKind::Content {
        files.push("newest.samples");
        if index == 0 {
            files.push("siteinfo.swdump");
        }
    }
    files.sort_unstable();
    files
...
pub(crate) fn target_file_inventory(
    ...
    kind: TargetKind,
    index: usize,
) -> Result<Vec<TargetCheckpointFile>, InvalidBuildState> {
    let mut files = Vec::new();
    for name in expected_target_files(kind, index) {
        let path = directory.join(name);
        let (bytes, identity) = identify_regular_file(&path).map_err(|error| {
            invalid(
                InvalidBuildKind::CorruptArtifact,
                &path,
                format!("cannot capture durable file identity: {error}"),
            )
        })?;
        files.push(TargetCheckpointFile {
            name: name.to_owned(),
            bytes,
            identity,
        });
...
pub(crate) fn make_target_checkpoint(
    ...
    let files = target_file_inventory(&partial, kind, index)?;
    if files.is_empty() {
        return Err(invalid(
            InvalidBuildKind::MissingArtifact,
            &partial,
...
fn inspect_target(
    ...
    let observed_files = target_file_inventory(&node, kind, index)?;
    if observed_files != receipt.files {
        return Err(invalid(
            InvalidBuildKind::ForeignIdentity,
            &receipt_path,
            "target file inventory does not match its receipt",
        ));
    }
    Ok(InspectedTarget {
        kind,
...
/// Establish receipt-level authority for retiring one committed source
/// target.  Cleanup callers validate every target before unlinking any of
/// them, so a late malformed or substituted target cannot turn a directory
/// name into deletion authority.
pub(crate) fn validate_ready_target_for_cleanup(
    root: &Path,
    plan: &DirectBuildPlan,
    kind: TargetKind,
    index: usize,
) -> Result<(), InvalidBuildState> {
    let inspected = inspect_target(root, plan, kind, index)?;
    if matches!(inspected.state, TargetState::Ready(_)) {
        Ok(())
    } else {
        Err(invalid(
            InvalidBuildKind::MissingArtifact,
            target_path(root, plan, kind, index)?,
            "committed source cleanup requires a ready target receipt",
        ))
    }
}
    ...
    match inspect_build(root, Some(&plan.plan_id))? {
        ...
        BuildState::Assembling { checkpoint, .. } => {
                ...
                .chain(checkpoint.unreceipted_segments.iter())
            {
                ...
                quarantine_uncommitted_entry(root, &path, "assembly-resume")?;
            }
...
pub(crate) fn commit_archive(
    ...
    match std::fs::symlink_metadata(&checkpoint) {
        Ok(_) => quarantine_uncommitted_entry(root, &checkpoint, "assembly-checkpoint")?,
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {}
        Err(error) => return Err(invalid(InvalidBuildKind::Io, checkpoint, error.to_string())),
    }
    match inspect_build(root, Some(&plan.plan_id))? {
...
mod tests {
    ...
    fn title_projection_bytes(fill: u8) -> Vec<u8> {
        let mut bytes = Vec::new();
        bytes.extend_from_slice(&crate::title_projection::PROJECTION_FILE_MAGIC);
        bytes.extend_from_slice(
            &crate::title_projection::PROJECTION_FILE_VERSION.to_le_bytes(),
        );
        bytes.extend_from_slice(
            &(crate::title_projection::PROJECTION_ENTRY_BYTES as u32).to_le_bytes(),
        );
        let mut entry = [0_u8; crate::title_projection::PROJECTION_ENTRY_BYTES as usize];
        entry[0] = fill;
    ...
    #[test]
    fn target_inventory_does_not_adopt_unowned_crash_residue() {
        let root = tempfile::tempdir().unwrap();
        for name in expected_target_files(TargetKind::History, 0) {
            std::fs::write(root.path().join(name), name.as_bytes()).unwrap();
        }
        let residue = root.path().join(".tmp-interrupted-sort");
        std::fs::create_dir(&residue).unwrap();
        std::fs::write(residue.join("foreign-user-input"), b"preserve").unwrap();

        let inventory = target_file_inventory(root.path(), TargetKind::History, 0).unwrap();
        assert_eq!(
            inventory
                .iter()
                .map(|file| file.name.as_str())
                .collect::<Vec<_>>(),
            vec!["data.swdump", "data.swframe", "title-records.swdump"]
        );
        assert_eq!(
            std::fs::read(residue.join("foreign-user-input")).unwrap(),
            b"preserve"
        );
    }
    ...
    fn title_projection_publish_boundaries_have_one_authoritative_interpretation() {
        ...
        let entry = title_projection_bytes(7);
        ...
        let orphan = title_projection_bytes(8);
        let orphan_identity = hex::encode(sha2::Sha256::digest(&orphan));
        std::fs::write(
            root.path()
    ...
    fn title_projection_receipt_rejects_structural_extent_change() {
        let root = tempfile::tempdir().unwrap();
        let plan = plan_with_content_targets(0);
        let entry = title_projection_bytes(3);
        let identity = hex::encode(sha2::Sha256::digest(&entry));
        let file_name = format!("title-projection-{identity}.entries");
        std::fs::write(root.path().join(&file_name), entry).unwrap();
    ...
    #[test]
    fn legacy_title_projection_requires_rebuild() {
        let root = tempfile::tempdir().unwrap();
        let plan = plan_with_content_targets(0);
        let entry = [3_u8; 16];
        let identity = hex::encode(sha2::Sha256::digest(entry));
        let file_name = format!("title-projection-{identity}.entries");
        std::fs::write(root.path().join(&file_name), entry).unwrap();
        let error = commit_title_projection(root.path(), &plan, &file_name, 1, &identity)
            .unwrap_err();
        assert_eq!(error.kind, InvalidBuildKind::UnsupportedSchema);
        assert!(error.diagnostic.contains("rebuild required"));
    }
