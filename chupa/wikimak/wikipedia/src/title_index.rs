...
const LEGACY_ENTRY_BYTES: usize = 16;
const UNKNOWN_NAMESPACE: i32 = i32::MIN;
...
pub(crate) struct Projection {
    ...
    pub(crate) candidates: Vec<TitleCandidate>,
}
...
pub struct TitleIndex {
    ...
    title_offset: usize,
    title_count: usize,
    title_entry_bytes: usize,
    explicit_namespaces: bool,
    frame_offset: usize,
    frame_count: usize,
...
fn write_index(
    ...
    for entry in &changes {
        ...
        temporary.write_all(&entry.namespace.to_le_bytes())?;
...
fn project_title_entries(
    ...
    let mut changes = BTreeMap::<(u64, u32), (u32, i32)>::new();
    for (title, time, page_id, namespace) in ownership_changes(&intervals) {
        changes.insert(
    ...
    Ok(changes
        .into_iter()
        .map(|((coded_title, time), (page_id, namespace))| TitleIndexEntry {
            coded_title,
            time,
            page_id,
            namespace,
    ...
    S: Iterator<Item = crate::archive::Result<SegmentIndexEntry>>,
{
    ...
    for entry in titles.by_ref() {
        if entry.namespace == UNKNOWN_NAMESPACE {
            return Err(crate::archive::ArchiveError::Invalid(
                "legacy hashed title has no namespace; rebuild required before update",
            ));
        }
...
impl TitleIndex {
    pub fn open(path: impl AsRef<Path>) -> crate::archive::Result<Self> {
        ...
        let (title_entry_bytes, explicit_namespaces) = match version {
            3 => (LEGACY_ENTRY_BYTES, false),
            FILE_VERSION => (ENTRY_BYTES, true),
            _ => {
                return Err(crate::archive::ArchiveError::Invalid(
                    "unknown title index format",
                ))
        ...
        if u32::from_le_bytes(bytes[12..16].try_into().expect("header bytes")) as usize
            != HEADER_BYTES
        {
            return Err(crate::archive::ArchiveError::Invalid(
                "unknown title index format",
        ...
        if explicit_namespaces {
            for position in 0..title_count {
                let start = title_offset + position * title_entry_bytes;
                if bytes[start + 20..start + 24].iter().any(|byte| *byte != 0) {
                    return Err(crate::archive::ArchiveError::Invalid(
                        "title index entry reserved bytes are nonzero",
                    ));
                }
            }
        }
    ...
    pub fn lookup(
        ...
        self.lookup_coded(coded_title(title, site_info), timestamp_micros)
    }

    /// Look up the owner of an already encoded title at a historical time.
    pub fn lookup_coded(&self, coded_title: u64, timestamp_micros: i64) -> Option<u64> {
        self.lookup_coded_at_seconds(coded_title, seconds(timestamp_micros))
    }

    /// Return the final owner mapping for an encoded title, or `None` when its
    /// final mapping is a tombstone.
    pub fn current_owner(&self, coded_title: u64) -> Option<u64> {
        self.lookup_coded_at_seconds(coded_title, u32::MAX)
    }

    fn lookup_coded_at_seconds(&self, coded_title: u64, time: u32) -> Option<u64> {
        let index = self
            .binary_search_by(|position| self.key_time(position).cmp(&(coded_title, time)));
        ...
        (stored_key == coded_title && page_id != 0).then_some(u64::from(page_id))
    }
    ...
    pub fn current_page_count(&self, namespace: Option<i32>) -> u64 {
        ...
        while position < self.len() {
            ...
            if current_page != 0
                && namespace.is_none_or(|wanted| self.namespace(end - 1) == wanted)
            {
                count += 1;
            }
            position = end;
    ...
    fn key_time(&self, position: usize) -> (u64, u32) {
        let start = self.title_offset + position * self.title_entry_bytes;
        let entry = &self.bytes[start..start + self.title_entry_bytes];
        (
    ...
    fn page_id(&self, position: usize) -> u32 {
        let start = self.title_offset + position * self.title_entry_bytes;
        let entry = &self.bytes[start..start + self.title_entry_bytes];
        u32::from_le_bytes(entry[12..16].try_into().expect("four page-id bytes"))
    }

    fn namespace(&self, position: usize) -> i32 {
        if self.explicit_namespaces {
            let start = self.title_offset + position * self.title_entry_bytes;
            i32::from_le_bytes(
            ...
            coded_namespace(self.key_time(position).0).unwrap_or(UNKNOWN_NAMESPACE)
        }
    }
    ...
    fn title_entry(&self, position: usize) -> TitleIndexEntry {
        ...
        TitleIndexEntry {
            ...
            namespace: self.namespace(position),
        }
    }
...
pub(crate) fn ownership_changes(intervals: &[Interval]) -> Vec<(&str, i64, u64, i32)> {
    ...
    while first < intervals.len() {
        ...
        events.sort_by_key(|(time, starts, start, page_id, namespace)| {
            (*time, *starts, *start, *page_id, *namespace)
        });
        let mut active = BTreeMap::<(i64, u64), (u32, i32)>::new();
        let mut owner = 0_u64;
        let mut position = 0;
        while position < events.len() {
            let time = events[position].0;
            while position < events.len() && events[position].0 == time {
                let (_, starts, start, page_id, namespace) = events[position];
                if starts {
                    active
                        .entry((start, page_id))
                        .and_modify(|(count, _)| *count += 1)
                        .or_insert((1, namespace));
                } else if let Some((count, _)) = active.get_mut(&(start, page_id)) {
                    *count -= 1;
                    if *count == 0 {
            ...
            let next_owner = active
                .last_key_value()
                .map_or((0, 0), |((_, page_id), (_, namespace))| (*page_id, *namespace));
            let (next_page_id, next_namespace) = next_owner;
            if next_page_id != owner {
                output.push((title, time, next_page_id, next_namespace));
                owner = next_page_id;
            }
        }
        first = last;
...
pub(crate) fn project_page_inputs(
    ...
    let state = states.first().map(|(at, title, namespace, deleted)| {
        (title.as_str(), *at, *namespace, *deleted)
    });
...
fn project_page(
    page_id: u64,
    current_title: Option<(&str, i64, Option<i64>, bool)>,
    actions: &[(i64, crate::archive::PageActionRecord)],
    site: &SiteInfoRecord,
) -> Projection {
    let mut title = None::<(String, i32)>;
    let mut exists = false;
    let mut since = i64::MIN;
    let mut closed = Vec::new();
    for (at, action) in actions {
        if exists && since < *at {
            if let Some((title, namespace)) = &title {
                closed.push(interval(title, *namespace, since, *at, page_id));
            }
        }
        let observed_title = full_title(action, site);
        let observed_namespace = action
            .namespace_at_event
            .and_then(|namespace| i32::try_from(namespace).ok())
            .unwrap_or_else(|| i32::try_from(split_title(&observed_title, site).0).unwrap_or(0));
        let observed = (observed_title, observed_namespace);
        match action.kind {
    ...
    let simulated = exists
        .then_some(title)
        .flatten()
        .map(|(title, namespace)| (title, namespace, since));
    let stated = current_title
        .filter(|(_, _, _, deleted)| !deleted)
        .map(|(title, at, namespace, _)| TitleCandidate {
            title: normalize(title),
            namespace: namespace
                .map(|value| i32::try_from(value).unwrap_or(0))
                .unwrap_or_else(|| i32::try_from(split_title(title, site).0).unwrap_or(0)),
            start: at,
        });
    ...
    } else if current_title.is_some_and(|(_, _, namespace, _)| namespace.is_some()) {
        candidates.extend(stated);
        candidates.extend(simulated.map(|(title, namespace, start)| TitleCandidate {
            title,
            namespace,
            start,
        }));
    ...
    candidates.retain(|candidate| !candidate.title.is_empty());
    Projection {
        page_id,
        closed,
        candidates,
...
fn assign_current_titles(projections: &[Projection]) -> crate::archive::Result<Vec<Interval>> {
    ...
    while let Some(index) = pending.pop_front() {
        ...
        let Some(previous) = owners.get(candidate.title.as_str()).copied() else {
            owners.insert(&candidate.title, index);
        ...
        let challenger_wins = (candidate.start, projections[index].page_id)
            > (previous_candidate.start, projections[previous].page_id);
        if challenger_wins {
    ...
    Ok(assigned
        ...
        .filter_map(|(index, choice)| {
            choice.map(|choice| {
                ...
                let candidate = &projection.candidates[choice];
                interval(
                    &candidate.title,
                    candidate.namespace,
                    candidate.start,
...
fn interval(title: &str, namespace: i32, start: i64, end: i64, page_id: u64) -> Interval {
    Interval {
        title: normalize(title),
        namespace,
        start,
...
mod tests {
    ...
    #[test]
    fn coded_lookup_matches_string_lookup_across_aliases_and_history() {
        let root = tempfile::tempdir().unwrap();
        let output = root.path().join("titles.swtitle");
        let site = site();
        let title = "Template:Shared";
        let alias = "T:Shared";
        let coded = coded_title(title, &site);
        let mut entries = vec![
            TitleIndexEntry {
                coded_title: coded,
                time: 10,
        ...
        entries.sort_unstable_by_key(|entry| (entry.coded_title, entry.time));
        write_generation_index(
            &output,
            &crate::generation::GenerationId::from_plan_id("coded-lookup-history"),
            entries.into_iter(),
            std::iter::empty::<crate::archive::Result<FrameIndexEntry>>(),
            std::iter::empty::<crate::archive::Result<SegmentIndexEntry>>(),
        )
        ...
        for (timestamp_micros, expected) in [
            (9_999_999, None),
            (10_000_000, Some(11)),
            (19_999_999, Some(11)),
            (20_000_000, Some(22)),
            ...
            let coded_lookup = index.lookup_coded(coded, timestamp_micros);
            assert_eq!(coded_lookup, expected);
            assert_eq!(index.lookup(title, timestamp_micros, &site), coded_lookup);
            assert_eq!(index.lookup(alias, timestamp_micros, &site), coded_lookup);
        }
    }

    #[test]
    fn current_owner_returns_final_mapping_and_hides_tombstones() {
        let root = tempfile::tempdir().unwrap();
        let output = root.path().join("titles.swtitle");
        let site = site();
        let live = coded_title("Current", &site);
        let deleted = coded_title("Template:Deleted", &site);
        let mut entries = vec![
            TitleIndexEntry {
                coded_title: live,
                time: 5,
        ...
        write_generation_index(
            &output,
            &crate::generation::GenerationId::from_plan_id("coded-lookup-current"),
            entries.into_iter(),
            std::iter::empty::<crate::archive::Result<FrameIndexEntry>>(),
            std::iter::empty::<crate::archive::Result<SegmentIndexEntry>>(),
        )
        ...
        assert_eq!(index.current_owner(live), Some(90));
        assert_eq!(index.current_owner(deleted), None);
        assert_eq!(index.current_owner(0), None);
    }
    ...
    #[test]
    fn current_page_count_uses_explicit_namespace_for_long_titles_moves_and_deletes() {
        let root = tempfile::tempdir().unwrap();
        let site = site();
        let main = "This is a deliberately long mainspace title whose key is hashed";
        let template = "Template:This is a deliberately long template title whose key is hashed";
        let mut entries = vec![
            TitleIndexEntry {
                coded_title: coded_title(main, &site),
                time: 10,
                page_id: 1,
                namespace: 0,
            },
            ...
            TitleIndexEntry {
                coded_title: coded_title(template, &site),
                time: 10,
                page_id: 2,
                namespace: 10,
            },
        ...
        entries.sort_unstable_by_key(|entry| (entry.coded_title, entry.time));
        let path = root.path().join("titles.swtitle");
        write_generation_index(
            &path,
            &crate::generation::GenerationId::from_plan_id("long-title-count"),
            entries.into_iter(),
            std::iter::empty::<crate::archive::Result<FrameIndexEntry>>(),
            std::iter::empty::<crate::archive::Result<SegmentIndexEntry>>(),
        )
        .unwrap();
        let index = TitleIndex::open(&path).unwrap();
        assert_eq!(index.current_page_count(Some(0)), 0);
        assert_eq!(index.current_page_count(Some(10)), 1);
        assert_eq!(index.current_page_count(None), 1);
    }

    #[test]
    fn v3_title_indexes_remain_readable_but_cannot_classify_hashed_namespaces() {
        let root = tempfile::tempdir().unwrap();
        let path = root.path().join("titles.swtitle");
        let site = site();
        let title = "Legacy";
        write_generation_index(
            &path,
            &crate::generation::GenerationId::from_plan_id("old-title-index"),
            std::iter::once(TitleIndexEntry {
                coded_title: coded_title(title, &site),
                time: 1,
                page_id: 7,
                namespace: 0,
            }),
        ...
        let mut legacy = current[..HEADER_BYTES].to_vec();
        legacy[8..12].copy_from_slice(&3_u32.to_le_bytes());
        let legacy_end = HEADER_BYTES + LEGACY_ENTRY_BYTES;
        legacy[48..56].copy_from_slice(&(legacy_end as u64).to_le_bytes());
        legacy[56..64].copy_from_slice(&(legacy_end as u64).to_le_bytes());
        legacy.extend_from_slice(&current[HEADER_BYTES..HEADER_BYTES + LEGACY_ENTRY_BYTES]);
        std::fs::write(&path, legacy).unwrap();

        let index = TitleIndex::open(&path).unwrap();
        assert_eq!(index.lookup(title, i64::MAX, &site), Some(7));
        assert_eq!(index.current_page_count(None), 1);
        assert_eq!(index.current_page_count(Some(0)), 1);
    }
}
