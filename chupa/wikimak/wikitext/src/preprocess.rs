        ...
        match b {
            b'{' => {
                ...
                if n < 2 {
                    ...
                    // MediaWiki disambiguates a run into nested openings,
                    // keeping a triple-brace argument innermost when the run
                    // is odd: 4 = 2+2, 5 = 2+3, 7 = 2+2+3.  Treating all
                    // five braces as one opening instead turns the real
                    // `{{{{{|safesubst:}}}#invoke:…}}` shape into literal
                    // outer braces after the argument closes.
                    let pairs = if n % 2 == 0 { n / 2 } else { (n - 3) / 2 };
                    let units = pairs + usize::from(n % 2 == 1);
                    if stack.len() + units <= MAX_PARSE_DEPTH {
                        for _ in 0..pairs {
                            stack.push(OpenB::new(OpenKind::Curly(2)));
                        }
                        if n % 2 == 1 {
                            stack.push(OpenB::new(OpenKind::Curly(3)));
                        }
                    } else {
                        // At the cap: keep the complete run as literal text.
                        // Partially opening it would let a later closing run
                        // consume only part of the source construct.
            ...
            b'[' if starts_external_link(&text[i + 1..]) => {
                // A single-bracket external link groups its URL and label.
                // In particular, `=` in a query string is not a named
                // template/parser-function argument separator.  A lone `[` is
                // otherwise ordinary text: templates use it as a #switch case
                // label, and opening a link there would swallow later pipes.
                if stack.len() < MAX_PARSE_DEPTH {
                    stack.push(OpenB::new(OpenKind::ExternalBracket));
                } else {
                    top_text(&mut stack, "[");
                }
                i += 1;
            ...
            b']' if matches!(stack.last().map(|o| o.kind), Some(OpenKind::ExternalBracket)) => {
                let open = stack.pop().unwrap();
                let parts: Vec<Part> = open.parts.into_iter().map(PartB::finish).collect();
                top_node(&mut stack, Node::ExternalLink(parts));
                i += 1;
            }
            b']' if i + 1 < len && bytes[i + 1] == b']' => {
...
fn starts_external_link(s: &str) -> bool {
    s.starts_with("//")
        || [
            "http://",
            "https://",
...
fn flatten_unmatched(stack: &mut [OpenB], open: OpenB) {
    let opener = match open.kind {
        ...
        OpenKind::ExternalBracket => "[".to_string(),
...
fn expand_body(ctx: &mut Ctx, text: &str, frame: &Frame, transcluding: bool) -> String {
    // Behavior switches can arrive from a transcluded template (for example
    // TOC limit emits `__TOC__` after its argument is expanded), so strip them
    // at every body boundary, not only from the top-level page text.  The
    // outer call still records the switches in Expanded; nested bodies only
    // need the same visible-output semantics here.
    let mut nested_switches = BehaviorSwitches::default();
    let t = strip_switches(&apply_inclusion(text, transcluding), &mut nested_switches);
    ...
    // Expansion can itself produce a behavior switch (for example through a
    // template argument or a Lua return value).  Remove that generated text
    // after expansion as well; nowiki/pre content is still represented by
    // protected markers at this point.
    let mut generated_switches = BehaviorSwitches::default();
    out = strip_switches(&out, &mut generated_switches);
...
fn expand_nodes(ctx: &mut Ctx, nodes: &[Node], frame: &Frame) -> String {
    ...
    for n in nodes {
        match n {
            ...
            Node::ExternalLink(parts) => out.push_str(&expand_external_link(ctx, parts, frame)),
        }
    }
    ctx.node_depth -= 1;
    out
...
fn expand_external_link(ctx: &mut Ctx, parts: &[Part], frame: &Frame) -> String {
    let inner: Vec<String> = parts.iter().map(|p| expand_part_full(ctx, p, frame)).collect();
    format!("[{}]", inner.join("|"))
}
...
fn expand_template(ctx: &mut Ctx, parts: &[Part], frame: &Frame) -> String {
    ...
    // A link cannot form a template title.  Citation modules deliberately
    // emit diagnostics such as `{{[[Template:Cite journal|cite journal]]}}`;
    // MediaWiki leaves the braces around that rendered link instead of
    // attempting to transclude a page whose title contains link markup.
    // Keep genuinely dynamic names (arguments/templates) on the expansion
    // path below, but preserve this invalid link-headed construct as text.
    if detected.is_none()
        && head
            .iter()
            .any(|node| matches!(node, Node::Link(_) | Node::ExternalLink(_)))
    {
        let inner = parts
            .iter()
            .map(|part| expand_part_full(ctx, part, frame))
            .collect::<Vec<_>>()
            .join("|");
        return format!("{{{{{inner}}}}}");
    }
    ...
    // Handle the save-time `subst:` marker, then strip runtime-transparent
    // prefixes (safesubst:/msg:/raw:, plus their localized siteinfo aliases)
    // and re-detect the remainder. When the remainder is itself dynamic,
    // expand it before re-detecting.
    loop {
        let Some((name, true, arg0)) = detected.as_ref() else { break };
        let key = resolve_magic(ctx.site, name.trim()).to_ascii_lowercase();
        if key == "subst" {
            // `subst:` is applied by MediaWiki's pre-save transform, not by
            // ordinary page rendering. Stored template bodies deliberately
            // use a literal subst call to detect accidental substitution;
            // expanding it here turns that guard into a false error.
            let inner = parts
                .iter()
                .map(|part| expand_part_full(ctx, part, frame))
                .collect::<Vec<_>>()
        ...
        if !matches!(key.as_str(), "safesubst" | "msg" | "raw" | "msgnw") {
            break;
        }
        let arg0 = arg0.clone();
    ...
    // runtime-prefix-stripped detection.
...
fn magic_word(ctx: &mut Ctx, upper: &str, arg: Option<&str>) -> Option<String> {
    match upper {
        ...
        "PAGESINCATEGORY" => {
            let category = resolve_title(arg.unwrap_or_default(), crate::title::NS_CATEGORY, ctx.site);
            return ctx
                .store
    ...
    for (idx, part) in rest.iter().enumerate() {
        ...
        match &part.name {
            Some(name_nodes) => {
                ...
                if case == "#default" {
                    let result = part_value_trim(ctx, part, frame);
                    // #default is both the fallback and an ordinary value
                    // destination for preceding bare cases.  Ruwiki's
                    // Publication template uses `| книга|#default = …`;
                    // skipping #default after `книга` matched incorrectly
                    // fell through into the later `статья` branch.
                    if matched {
                        return result;
                    }
                    default_val = Some(result);
                    continue;
                }
...
fn pf_invoke(ctx: &mut Ctx, arg0: &[Node], rest: &[Part], frame: &Frame) -> String {
    ...
    match invoker.invoke(&module, &function, &inv_frame, ctx.store) {
        // A Scribunto return value is wikitext inserted at the #invoke call
        // site.  It therefore re-enters preprocessing in the current frame:
        // modules commonly return template calls and safesubst parser
        // functions.  The live node-depth guard also bounds a module that
        // returns another invocation recursively.
        Ok(s) => expand_body(ctx, &s, frame, false),
        Err(e) => {
            ctx.misses.failed_invokes.push(format!("Module:{module}#{function}"));
...
fn pf_time(ctx: &mut Ctx, arg0: &[Node], rest: &[Part], frame: &Frame) -> String {
    ...
    format_time(format, &civ, ctx.site)
}
...
fn format_time(fmt: &str, c: &magic::Civil, site: &SiteConfig) -> String {
    let mut out = String::new();
    ...
    while i < chars.len() {
        ...
        match ch {
            ...
            'x' if chars.get(i + 1) == Some(&'g') => {
                out.push_str(month_name_genitive(site, c.month));
                i += 2;
                continue;
            }
            ...
            'M' => out.push_str(month_name_abbreviated(site, c.month)),
            'F' => out.push_str(month_name_nominative(site, c.month)),
...
fn month_name_genitive(site: &SiteConfig, month: u32) -> &'static str {
    const RUSSIAN_GENITIVE: [&str; 12] = [
        "января",
        "февраля",
        "марта",
        "апреля",
    ...
    if site_language_is(site, "ru") {
        RUSSIAN_GENITIVE
            .get(month.saturating_sub(1) as usize)
            .copied()
            .unwrap_or("января")
    } else {
        magic::month_name(month)
    }
}

fn month_name_nominative(site: &SiteConfig, month: u32) -> &'static str {
    const RUSSIAN_NOMINATIVE: [&str; 12] = [
        "январь",
        "февраль",
        "март",
        "апрель",
...
fn month_name_abbreviated(site: &SiteConfig, month: u32) -> &'static str {
    const RUSSIAN_ABBREVIATED: [&str; 12] = [
    ...
    if site_language_is(site, "ru") {
        ...
        &magic::month_name(month)[..3]
    }
}

fn site_language_is(site: &SiteConfig, expected: &str) -> bool {
    site.lang
        .split(['-', '_'])
        .next()
        .unwrap_or(&site.lang)
...
fn parse_iso_datetime(s: &str) -> Option<magic::Civil> {
    ...
    if !(2..=3).contains(&dp.len()) {
        return None;
    }
    let dotted_day_month_year = dp.len() == 3
        && date.matches('.').count() == 2
        && dp[0].len() <= 2
        && dp[2].len() == 4;
    let (year, month, day) = if dotted_day_month_year {
        (
            dp[2].parse::<i64>().ok()?,
            dp[1].parse::<u32>().ok()?,
            dp[0].parse::<u32>().ok()?,
...
fn tokenize_expr(s: &str) -> Result<Vec<Tk>, String> {
    // MediaWiki accepts the typographic minus emitted by many localized
    // number templates, and treats a non-breaking space as whitespace.  A
    // byte-wise lexer otherwise reports the first UTF-8 byte as `â`, hiding
    // both the input and the actual compatibility gap.
    let normalized = s.replace('\u{2212}', "-").replace('\u{00a0}', " ");
...
fn parse_redirect_with_site(text: &str, site: &SiteConfig) -> Option<String> {
    parse_redirect_if(text, |word| {
        if word.eq_ignore_ascii_case("redirect")
            || resolve_magic(site, &format!("#{word}")).eq_ignore_ascii_case("#redirect")
        {
            return true;
        }
        // Archives created before magic-word capture have an empty alias
        // table. Preserve redirect semantics for the local mirror languages
        // that such archives already contain; newly captured siteinfo remains
        // the general source of localized aliases.
        match site.lang.split(['-', '_']).next().unwrap_or(&site.lang) {
            "ru" => word.to_lowercase() == "перенаправление",
            "lv" => word.to_lowercase() == "pāradresācija",
            _ => false,
        }
    })
}
