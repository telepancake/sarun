...
impl<'a> Ctx<'a> {
    ...
    /// Pull `<!-- -->`, `<nowiki>`, `<pre>`, `<ref>`, and `<gallery>` out
    /// before any block/inline processing. Protected bodies become markers;
    /// gallery entries are parsed into safe media markup first so their
    /// captions can still use the ordinary inline renderer.
    fn strip_extension_tags(&mut self, s: &str) -> String {
        ...
        while i < b.len() {
            if b[i] == b'<' {
                ...
                if let Some((attrs, self_closing, past)) = match_ext_tag(s, i, "gallery") {
                    let (inner, next) = if self_closing {
                        ("", past)
                    } else {
                        read_to_close(s, past, "gallery")
                    };
                    let gallery_html = self.render_gallery(attrs, inner);
                    let m = self.strip.stash_block(gallery_html);
                    out.push_str(&m);
                    i = next;
                    continue;
                }
    ...
    fn render_internal_link(&mut self, inner: &str, trail: &str) -> String {
        ...
        if !leading_colon && title.ns == NS_FILE {
            return self.render_image(&title, &parts, None);
        }
    ...
    fn render_image(
        ...
        let caption_text = cap_html.as_deref().map(strip_rendered_tags);
        let alt_text = alt
            .as_deref()
            .map(|a| self.image_plain_text(a))
            .or(caption_text.clone())
            .unwrap_or_else(|| title.text.clone());
        let req_width = width.or(request_width).or(if format == "thumb" || format == "frameless" {
            Some(220)
        } else {
            None
        });
        ...
        let title_attr = if !is_boxed {
            caption_text
                .as_deref()
                .map(|text| format!(" title=\"{}\"", html::escape_attr(text)))
                .unwrap_or_default()
        } else {
            String::new()
        };
        ...
        let inner_media = match &src {
            Some(url) => {
                if Self::audio_mime(&title.text).is_some() {
                    format!(
                        "<audio controls src=\"{}\"></audio>",
                        html::escape_attr(url)
                    )
                } else {
    ...
    /// Render the body of an HTML gallery. Gallery filenames are parsed as
    /// File titles even when the source omits `File:`/`Image:`; the resulting
    /// image itself still goes through `render_image`, so media lookup,
    /// placeholders, miss accounting, and strip-marker handling stay shared
    /// with ordinary file links.
    fn render_gallery(&mut self, attrs: &str, inner: &str) -> String {
        let width_hint = gallery_dimension(attrs, "widths")
            .or_else(|| gallery_dimension(attrs, "heights"));
        let mut items = String::new();

        for line in inner.lines() {
            let parts = split_pipe(line);
            let Some(raw_name) = parts.first().map(|name| name.trim()) else {
                continue;
            };
            let Some(title) = self.gallery_file_title(raw_name) else {
                continue;
            };

            let mut image_parts = vec![title.prefixed(self.site), "frameless".to_string()];
            let mut caption_parts = Vec::new();
            for option in &parts[1..] {
                if gallery_image_option(option) {
                    // `render_image` already knows how to turn alt text into
                    // safe plain text. Other gallery image options are
                    // presentation controls and do not belong in the
                    // visible caption.
                    if option.trim().to_ascii_lowercase().starts_with("alt=") {
                        image_parts.push(option.clone());
                    }
                } else {
                    caption_parts.push(option.as_str());
                }
            }

            let image_marker = self.render_image(&title, &image_parts, width_hint);
            let image_html = self.strip.resolve(&image_marker);
            let caption = if caption_parts.is_empty() {
                String::new()
            } else {
                self.inline(&caption_parts.join("|"))
            };
            items.push_str(&format!(
                "<li class=\"gallerybox\"><div class=\"thumb\">{image_html}</div><div class=\"gallerytext\">{caption}</div></li>"
            ));
        }
        ...
        if items.is_empty() {
            ...
            format!("<ul class=\"gallery mw-gallery-default\">{items}</ul>")
        }
    }

    /// Resolve a gallery line using the same namespace/title rules as a
    /// wikilink. Known non-file namespaces are not silently turned into file
    /// names; bare names are the normal gallery shorthand for File titles.
    fn gallery_file_title(&self, raw: &str) -> Option<Title> {
        if raw.is_empty()
            || raw.starts_with("<!--")
            || raw.starts_with("[[")
            || raw.starts_with("{{")
        ...
        let parsed = Title::parse(raw, self.site);
        let title = if parsed.ns == NS_FILE {
            parsed
        } else if raw.split_once(':').is_some_and(|(prefix, _)| {
            crate::title::resolve_ns(prefix, self.site).is_some()
        }) {
            return None;
        } else {
            Title::parse(&format!("File:{raw}"), self.site)
        };
        (title.ns == NS_FILE && !title.text.is_empty()).then_some(title)
    }

fn audio_mime(name: &str) -> Option<&'static str> {
    match name.rsplit_once('.').map(|(_, ext)| ext.to_ascii_lowercase()) {
        Some(ext) if ext == "ogg" || ext == "oga" => Some("audio/ogg"),
        _ => None,
    }
}

    /// Render image option text through the normal inline parser, then keep
    /// only its text for an HTML attribute. Links and formatting are valid in
    /// captions, but their generated tags must not become literal alt/title
    /// text.
    fn image_plain_text(&mut self, source: &str) -> String {
        strip_rendered_tags(&self.inline(source))
    }
...
fn find_link(s: &str, open: usize) -> Option<(&str, usize, &str)> {
    ...
    while i + 1 < b.len() {
        if external {
            if b[i] == b'[' && b[i + 1] == b'[' {
                // An internal link may be used as the label of an external
                // link. Its closing `]]` belongs to that nested link, not to
                // the surrounding external link or image.
                depth += 1;
                i += 2;
            } else if b[i] == b']' && b[i + 1] == b']' && depth > 1 {
                depth -= 1;
                i += 2;
            } else if b[i] == b']' {
                external = false;
...
/// Remove tags from already-rendered inline markup, retaining the serialized
/// text/character references for a later attribute escape.
fn strip_rendered_tags(s: &str) -> String {
    let b = s.as_bytes();
    let mut out = String::with_capacity(s.len());
    let mut i = 0;
    while i < b.len() {
...
/// Split link inner on top-level `|` (respecting nested `[[…]]`/`{{…}}` and
/// single-bracket external links).
fn split_pipe(s: &str) -> Vec<String> {
    ...
    while i < b.len() {
        ...
        if b[i] == b'[' {
            ...
            continue;
        } else if b[i] == b']' && b.get(i + 1) == Some(&b']') && depth_sq > 0 {
            depth_sq -= 1;
            i += 2;
            continue;
        }
...
fn gallery_dimension(attrs: &str, wanted: &str) -> Option<u32> {
    html::parse_attrs(attrs).into_iter().find_map(|(name, value)| {
...
fn gallery_image_option(option: &str) -> bool {
    let low = option.trim().to_ascii_lowercase();
    matches!(
        low.as_str(),
        "thumb"
            | "thumbnail"
...
fn ref_attrs(attrs: &str) -> (String, String) {
    ...
    // A template can pass an already-quoted value to #tag.  Its generated
    // attribute is then name=""foo""; the generic HTML attribute scanner
    // sees the first pair of quotes as an empty value and loses `foo`.
    // Recover that Cite-specific spelling so a later empty-body use still
    // resolves to the named reference.
    if name.is_empty() {
        name = doubled_quoted_ref_attr(attrs, "name").unwrap_or_default();
    }
    if group.is_empty() {
        group = doubled_quoted_ref_attr(attrs, "group").unwrap_or_default();
    }
    (name.trim().to_string(), group.trim().to_string())
}

fn doubled_quoted_ref_attr(attrs: &str, wanted: &str) -> Option<String> {
    ...
    while i < b.len() {
        ...
        if !key.eq_ignore_ascii_case(wanted) || b.get(i) != Some(&b'=') {
            continue;
        }
        i += 1;
        while i < b.len() && b[i].is_ascii_whitespace() {
        ...
        let quote = *b.get(i)?;
        if (quote != b'"' && quote != b'\'') || b.get(i + 1) != Some(&quote) {
            continue;
        }
        let value_start = i + 2;
        let mut end = value_start;
        while end + 1 < b.len() {
            if b[end] == quote && b[end + 1] == quote {
                return Some(attrs[value_start..end].to_string());
            }
            end += 1;
        }
        return None;
