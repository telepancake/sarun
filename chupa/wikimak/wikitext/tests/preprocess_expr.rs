...
#[test]
fn ifexpr_does_not_expand_unselected_error_branch() {
    let s = MockStore::new();
    assert_eq!(
        xt(&s, "{{#ifexpr:1|selected|{{#expr:1/0}}}}"),
        "selected"
    );
}

#[test]
fn ifexpr_expands_only_the_selected_branch() {
    let s = MockStore::new();
    assert_eq!(
        xt(&s, "{{#ifexpr:1|{{#expr:2+2}}|{{#expr:1/0}}}}"),
        "4"
    );
}

#[test]
fn nested_ifexpr_error_branch_does_not_enter_outer_expr() {
    let s = MockStore::new();
    assert_eq!(
        xt(&s, "{{#expr:{{#ifexpr:1|2|<span class=\"error\">bad</span>}}+1}}"),
        "3"
    );
}
