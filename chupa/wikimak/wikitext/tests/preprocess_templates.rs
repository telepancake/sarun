...
#[test]
fn safesubst_expands_but_subst_waits_for_pre_save_transform() {
    let mut s = MockStore::new();
    s.template("G", "Hi {{{1}}}");
    assert_eq!(xt(&s, "{{subst:G|there}}"), "{{subst:G|there}}");
    assert_eq!(xt(&s, "{{safesubst:G|you}}"), "Hi you");
}

#[test]
fn literal_subst_guard_in_transcluded_body_is_not_a_false_error() {
    let mut s = MockStore::new();
    s.template("Substcheck", "SUBST");
    s.template(
        "Guarded",
        "{{Wrap|subst=<includeonly>{{subst:Substcheck}}</includeonly>}}",
    );
    s.template("Wrap", "{{#ifeq:{{{subst}}}|SUBST|error|ok}}");
    assert_eq!(xt(&s, "{{Guarded}}"), "ok");
}

#[test]
fn behavior_switch_emitted_by_transcluded_template_is_removed() {
    let mut s = MockStore::new();
    s.template(
        "TOC limit",
        "<div class=\"toc-limit-{{{1|3}}}\">__TOC__</div>",
    );
    let out = xt(&s, "{{TOC limit|3}}");
    assert_eq!(out, "<div class=\"toc-limit-3\"></div>");
    assert!(!out.contains("__TOC__"));
}

#[test]
fn safesubst_parser_function_name_is_redispatched() {
    let s = MockStore::new();
    assert_eq!(
        xt(&s, "{{safesubst:#if:1|Selected|Other}}"),
        "Selected"
    );
}

#[test]
fn russian_old_style_date_template_shape_avoids_time_error() {
    let mut s = MockStore::new();
    ...
    s.template(
        "OldStyleDate3",
        "{{#ifexpr: {{#time:Y|{{{1}}}}}<2100 | {{#time:j\"&nbsp;\"xg|{{{1}}}}} | future }}",
    );
    let out = xt(&s, "{{Сс3|22.10.1721}}");
    assert!(!out.contains("class=\"error\""), "got: {out}");
    assert_eq!(out, "22&nbsp;октября");
}
