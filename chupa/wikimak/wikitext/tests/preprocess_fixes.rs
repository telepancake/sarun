...
// --- bug 1: safesubst prefix + inner dispatch ------------------------------

#[test]
fn safesubst_invoke_static_and_dynamic() {
    let s = MockStore::new();
    // Static: safesubst prefix strips, `#invoke:String` dispatches (String is the
    // module, the next part the function).
    assert_eq!(xt_invoke(&s, "{{safesubst:#invoke:String|len|abc}}"), "[String/len]");
    // Dynamic name via `{{{|safesubst:}}}` — use the exact five-opening-brace
    // spelling stored by templates such as ruwiki's Dead link. Whitespace
    // after the outer `{{` changes brace disambiguation and does not exercise
    // this case. The expanded name must redispatch as #invoke.
    assert_eq!(xt_invoke(&s, "{{{{{|safesubst:}}}#invoke:String|len|abc}}"), "[String/len]");
}

#[test]
fn subst_waits_for_pre_save_transform_for_templates_and_functions() {
    let mut s = MockStore::new();
    s.template("Foo", "BODY");
    // `subst:` is MediaWiki's save-time transform. Runtime parsing preserves
    // the call whether its target is a template or a parser function.
    assert_eq!(xt(&s, "{{subst:Foo}}"), "{{subst:Foo}}");
    assert_eq!(
        xt(&s, "{{subst:#if:x|yes|no}}"),
        "{{subst:#if:x|yes|no}}"
    );
...
#[test]
fn legacy_ru_archive_without_magicword_table_still_follows_redirects() {
    let mut s = MockStore::new();
    s.site.lang = "ru".to_string();
    s.site.namespaces.get_mut(&10).unwrap().localized = "Шаблон".to_string();
    assert!(s.site.magic_aliases.is_empty());
    s.template("Alias", "#Перенаправление [[Шаблон:Target]]");
    s.template("Target", "localized");
    assert_eq!(xt(&s, "{{Alias}}"), "localized");
}
