...
each corpus's shape wants, served through Chupa. Sarun consumes Chupa as a
library and supplies optional host integrations. Three mirrors first:
...
- **Fetch**: Chupa drivers own ordinary host-side retrieval. An embedding
  application may place those processes inside a stronger network sandbox,
  but Chupa does not depend on such a sandbox.
- **Serve**: Wikipedia reads frames directly from the archive; other mirrors
  ...
  with no checkout.
- **Update**: incremental by design — chains prepend (newest-first; the new head is frame 0). Scheduled by
  Chupa (`src/supervisor.rs`, the `chupa` CLI, and its TUI): jobs in
  `{state_home}/mirrors.db`, a minute tick starts due
  ...
  Chupa binary (multi-call dispatch: `chupa gitdepot|wikimak|ietfmak …`
  or an argv[0] symlink); a run spawns Chupa's own binary in driver
  mode. Interrupted runs surface as
...
- **Portable Wikipedia libraries**: each `.swdump` is self-identifying through
  ...
  TUI, `v` opens a registered archive. Chupa validates the
