...
fn update_io_is_bounded_not_o_history() {
    ...
    fn written_bytes() -> u64 {
        ...
        // `wchar` is the number of bytes submitted by write-family calls.
        // Unlike `write_bytes`, it remains observable when the test runs on
        // a virtiofs mount whose physical writes are charged to the host.
        io.lines().find_map(|l| l.strip_prefix("wchar: "))
