...
fn require_document_bound(label: &str, bytes: usize) -> anyhow::Result<()> {
    if bytes as u64 > MAX_DOC_BYTES {
        anyhow::bail!(
            "reader: {label} is {bytes} bytes (cap {MAX_DOC_BYTES})"
        );
    }
    Ok(())
}
...
const WIKI_REVISION_SUMMARY_CACHE_BYTES: usize = 1 << 20;
const WIKI_REVISION_RECORD_PAYLOAD_BYTES: usize = MAX_DOC_BYTES as usize + (1 << 20);
...
fn wiki_page_html(
    ...
    let (pid, resolved, text) = match page_id {
        Some(page_id) => {
...
struct WikiReader {
    ...
    page_id: u64,
    page_title: String,
    revisions: Option<wikimak_wikipedia::archive_browse::PageRevisionSummaryCursor>,
    revision_texts: Option<wikimak_wikipedia::archive_browse::PageRevisionTextCursor>,
...
impl Reader {
    fn new(source: Source, raw: Vec<u8>, kind: Kind, display: String) -> anyhow::Result<Reader> {
        require_document_bound(&display, raw.len())?;
    ...
    pub fn open_wiki(root: PathBuf, title: Option<String>) -> anyhow::Result<Reader> {
        ...
        let mut revisions = archive
            .page_revision_summary_cursor(
                page_id,
                WIKI_REVISION_SUMMARY_CACHE_BYTES,
                WIKI_REVISION_RECORD_PAYLOAD_BYTES,
            )
        ...
        revisions
            .summary(0)
            .map_err(|e| anyhow::anyhow!("wiki page history: {e}"))?
            .ok_or_else(|| anyhow::anyhow!("wiki page has no revisions"))?;
        let revision_texts = archive
            .page_revision_text_cursor_bounded(
                page_id,
                WIKI_REVISION_TEXT_CACHE_BYTES,
                WIKI_REVISION_RECORD_PAYLOAD_BYTES,
            )
        ...
        reader.wiki = Some(WikiReader {
            ...
            revision_texts: Some(revision_texts),
    ...
    fn load_into(&mut self, source: Source) -> anyhow::Result<()> {
        ...
        if let Some((page_id, revisions, revision_texts)) = wiki_page {
            ...
            wiki.revisions = Some(revisions);
            wiki.revision_texts = Some(revision_texts);
            self.history_selected = match &self.source {
                Source::Wiki {
                    ...
                    .revisions
                    .as_mut()
                    .expect("wiki page installed a revision cursor")
                    .find_at_or_before(*timestamp)
                    .map_err(|error| anyhow::anyhow!("wiki page history: {error}"))?
                    .unwrap_or(0),
                _ => 0,
            };
    ...
    fn focus_history(&mut self) -> bool {
        let count = match self.ensure_revision_count() {
            Ok(count) => count,
            Err(error) => {
    ...
    fn apply_wiki_view(&mut self, revision_index: usize) -> anyhow::Result<()> {
        ...
        let (archive, page_title, timestamp_micros, page_id, mode, current, previous) = {
            ...
            let revision = wiki
                .revisions
                .as_mut()
                .ok_or_else(|| anyhow::anyhow!("wiki page history is not open"))?
                .summary(revision_index)
                .map_err(|error| anyhow::anyhow!("wiki page history: {error}"))?
            ...
            if revision.text_len > MAX_DOC_BYTES {
                anyhow::bail!(
                    "reader: revision {} text is {} bytes (cap {MAX_DOC_BYTES})",
                    revision.revision_id,
                    revision.text_len,
                );
            }
            let timestamp_micros = revision.timestamp_micros;
            let has_previous = if wiki.view_mode == WikiViewMode::Diff {
                wiki.revisions
                    .as_mut()
                    .expect("revision cursor checked above")
                    .summary(revision_index + 1)
                    .map_err(|error| anyhow::anyhow!("wiki page history: {error}"))?
                    .is_some_and(|previous| {
                        previous.has_text && previous.text_len <= MAX_DOC_BYTES
                    })
            } else {
                false
            };
            let current = wiki
                .revision_texts
                .as_mut()
                .ok_or_else(|| anyhow::anyhow!("wiki revision stream is not open"))?
                .text(revision_index)
            ...
            let previous = if has_previous {
                wiki.revision_texts
                    .as_mut()
                    .expect("revision text cursor checked above")
    ...
    fn open_history_revision(&mut self) {
        let revision_id = match self.revision_summary(self.history_selected) {
            Ok(Some(revision)) => revision.revision_id,
            Ok(None) => {
    ...
    fn selected_contributor(&mut self) -> Option<wikimak_wikipedia::ContributorMeta> {
        self.revision_summary(self.history_selected)
            .ok()
            .flatten()
            .map(|revision| revision.contributor)
    }
    ...
    pub fn binding_context(&self) -> crate::ui::ReaderBindingContext {
        if self.searching {
            crate::ui::ReaderBindingContext::Search
    ...
    pub fn handle_key(&mut self, code: crossterm::event::KeyCode) -> KeyResult {
        ...
        match action {
            ...
            crate::ui::ReaderAction::HistoryEnd => {
                match self.ensure_revision_count() {
                    Ok(count) => {
                        self.history_selected = count.saturating_sub(1);
                        self.move_history(0);
                    }
                    Err(error) => self.status = error.to_string(),
                }
    ...
    pub fn page_history_lines(&mut self, limit: usize) -> Vec<Line<'static>> {
        ...
        let Some(revisions) = wiki.revisions.as_mut() else {
            return vec![Line::from(Span::styled(
                "(page history is unavailable for this view)",
                Style::default().add_modifier(Modifier::DIM),
            ))];
        };
        if let Some(count) = revisions.known_count() {
            if count == 0 {
                return vec![Line::from(Span::styled(
                    "(no revisions)",
                    Style::default().add_modifier(Modifier::DIM),
            ...
            self.history_selected = self.history_selected.min(count - 1);
            self.history_scroll = self
                .history_scroll
                .min(count.saturating_sub(self.history_view_h));
        }
        if self.history_selected < self.history_scroll {
            self.history_scroll = self.history_selected;
        ...
        let displayed = matches!(self.source, Source::Wiki { .. })
            .then_some(self.history_selected);
        let mut visible = Vec::new();
        for index in self.history_scroll..self.history_scroll.saturating_add(self.history_view_h) {
            match revisions.summary(index) {
                Ok(Some(revision)) => visible.push((index, revision)),
                Ok(None) => break,
                Err(error) => {
                    return vec![Line::from(Span::styled(
                        format!("page history: {error}"),
                        Style::default().fg(Color::Red),
                    ))];
                }
            }
        }
        if visible.is_empty() && revisions.known_count() == Some(0) {
            return vec![Line::from(Span::styled(
                "(no revisions)",
                Style::default().add_modifier(Modifier::DIM),
            ))];
    ...
    fn revision_summary(
        &mut self,
        index: usize,
    ) -> anyhow::Result<Option<wikimak_wikipedia::archive_browse::PageRevisionSummary>> {
        let Some(revisions) = self
            .wiki
            .as_mut()
            .and_then(|wiki| wiki.revisions.as_mut())
        else {
            return Ok(None);
        };
        revisions
    ...
    fn ensure_revision_count(&mut self) -> anyhow::Result<usize> {
        let Some(revisions) = self
            .wiki
            .as_mut()
            .and_then(|wiki| wiki.revisions.as_mut())
        ...
        revisions.scan_count().map_err(|error| anyhow::anyhow!("wiki page history: {error}"))
    }

    pub fn revision_count(&self) -> usize {
        self.wiki
            .as_ref()
            .and_then(|wiki| wiki.revisions.as_ref())
            .map_or(0, |revisions| {
                revisions
                    .known_count()
                    .unwrap_or_else(|| revisions.observed_count())
            })
    }

    pub fn revision_count_label(&self) -> String {
        let Some(revisions) = self
            .wiki
            .as_ref()
            .and_then(|wiki| wiki.revisions.as_ref())
        ...
        revisions.known_count().map_or_else(
            || format!("{}+", revisions.observed_count()),
    ...
    pub fn handle_history_mouse(
        ...
        if self
            ...
            .as_ref()
            .and_then(|wiki| wiki.revisions.as_ref())
            .and_then(|revisions| revisions.known_count())
...
mod tests {
    ...
    #[test]
    fn every_reader_source_obeys_the_document_byte_bound() {
        let bytes = vec![b'x'; usize::try_from(MAX_DOC_BYTES).unwrap() + 1];
        let error = match Reader::new(
            Source::File(PathBuf::from("oversized.internal")),
            bytes,
            Kind::Text,
            "oversized internal source".into(),
        ) {
            Ok(_) => panic!("oversized internal source must be rejected"),
            Err(error) => error,
        };
        assert!(error.to_string().contains("cap 16777216"), "{error}");
    }
    ...
    fn history_pane_focuses_and_opens_selected_revision() {
        ...
        assert!(!reader.history_focused(), "{}", reader.status);
    ...
    fn wiki_raw_and_diff_views_use_streaming_history_cursor() {
        ...
        let revisions = reader
            ...
            .as_mut()
            .unwrap();
        assert_eq!(revisions.known_count(), None);
        assert_eq!(revisions.observed_count(), 1, "open reads only the head summary");
        assert!(revisions.summary(0).unwrap().unwrap().has_text);
        assert!(revisions.summary(1).unwrap().unwrap().has_text);
        assert_eq!(revisions.known_count(), None, "ordinary access does not count all revisions");
        assert!(revisions.cache_entries() <= 8);
        assert!(revisions.cache_bytes() <= WIKI_REVISION_SUMMARY_CACHE_BYTES);

        reader.handle_key(KeyCode::Char('h'));
        assert_eq!(reader.revision_count(), 2, "history focus performs the explicit count scan");
        reader.handle_key(KeyCode::Char('2'));
        assert!(
            String::from_utf8_lossy(&reader.raw).contains("[[Beta Article]]"),
            "raw mode preserves original link markup; status={}; raw={}",
            reader.status,
