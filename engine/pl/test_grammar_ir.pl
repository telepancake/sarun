...
tree_sitter_fixture(
    grammar(source(text(utf8)), expression,
            [rule(identifier,
             ...
             rule(comment,
                  terminal(text(regex("//[^\\n]*")),
                           presentation([meta(syntax, comment)]))),
             rule(expression,
                  extras([ref(comment)],
                         conflicts(
                             [expression],
            ...
            [rule(packet,
                  seq([field(length,
                             terminal(bytes(uint(32, little)),
                                      presentation([meta(syntax, length)]))),
                       field(kind,
                             terminal(bytes(uint(8, big)),
                                      presentation([meta(syntax, tag)]))),
                       field(payload,
                             terminal(bytes(slice(value(length))),
                                      presentation([meta(syntax, payload)]))),
                       dispatch(value(kind),
                       ...
                       field(checksum,
                             terminal(bytes(uint(32, big)),
                                      presentation([meta(syntax, checksum)]))),
             ...
             rule(text_payload,
                  terminal(text(class(utf8)),
                           presentation([meta(syntax, text)]))),
             rule(nested_payload,
                  embed(grammar_ref(nested_protocol), bounded(value(length)))),
             rule(opaque_payload,
                  terminal(bytes(rest),
                           presentation([meta(syntax, opaque)])))],
            [])).
