...
pub fn prepare_early_tap() -> Result<bool> {
    ...
    unsafe {
        std::env::remove_var(EARLY_TAP_ENV);
    }
    if marker != "1" {
    ...
    if unsafe { libc::unshare(libc::CLONE_NEWUSER | libc::CLONE_NEWNET) } != 0 {
        bail!(
            "unshare(CLONE_NEWUSER|CLONE_NEWNET) for rootless Tap: {}",
...
unsafe fn probe_tap_child() -> bool {
    unsafe {
        if libc::unshare(libc::CLONE_NEWNET) != 0 {
            ...
            if !write_id_map(b"/proc/self/uid_map\0", libc::getuid()) {
                return false;
            }
            if !write_id_map(b"/proc/self/gid_map\0", libc::getgid()) {
                return false;
            }
            if libc::unshare(libc::CLONE_NEWNET) != 0 {
                return false;
...
pub fn create_netns_tap() -> Result<OwnedFd> {
    if let Some(tap) = EARLY_TAP.get().and_then(|slot| slot.lock().unwrap().take()) {
        return Ok(tap);
    }
    unshare_netns()?;
...
pub fn configure_appliance_network(mode: crate::generated_wire::NetMode) -> Result<()> {
    ...
    let (address, prefix, gateway, neighbor) = match mode {
        ...
        crate::generated_wire::NetMode::Host => ([10, 0, 2, 15], 24, [10, 0, 2, 2], None),
    };
...
pub fn unshare_netns() -> Result<()> {
    ...
    if unsafe { libc::unshare(libc::CLONE_NEWUSER) } != 0 {
        bail!(
            "unshare(CLONE_NEWUSER) (for rootless tap): {}",
            std::io::Error::last_os_error()
        );
    }
    write_identity_maps(uid, gid)?;
    if unsafe { libc::unshare(libc::CLONE_NEWNET) } != 0 {
        bail!(
            "unshare(CLONE_NEWNET) after userns: {}",
            std::io::Error::last_os_error()
        );
    }
    Ok(())
...
fn open_tap(name: &str) -> Result<OwnedFd> {
    ...
    if nb.len() >= req.name.len() {
        bail!("tap name too long");
    }
    ...
    if r < 0 {
        bail!("TUNSETIFF: {}", std::io::Error::last_os_error());
    }
    Ok(owned)
}
...
fn bring_link_up(name: &str) -> Result<()> {
    set_link_up(name)
}
...
fn add_neigh(ip: [u8; 4], mac: [u8; 6], ifname: &str) -> Result<()> {
    ...
    for i in 0..6 {
        a.arp_ha.sa_data[i] = mac[i] as libc::c_char;
    }
    ...
    if rc < 0 {
        bail!("SIOCSARP {:?}: {}", ip, std::io::Error::last_os_error());
    }
    Ok(())
}
...
fn assign_ip(name: &str, ip: [u8; 4], prefix: u8) -> Result<()> {
    ...
    req.addr.sin_addr = libc::in_addr {
        s_addr: u32::from_ne_bytes(ip),
    };
    const SIOCSIFADDR: libc::c_ulong = 0x8916;
    let r = unsafe { libc::ioctl(s.as_raw_fd(), SIOCSIFADDR as _, &req) };
    if r < 0 {
        bail!("SIOCSIFADDR({name}): {}", std::io::Error::last_os_error());
    }
    // Netmask.
    let mut req: IfreqAddr = unsafe { std::mem::zeroed() };
    write_name(&mut req.name, name)?;
