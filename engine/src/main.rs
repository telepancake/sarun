...
fn serve() -> i32 {
    ...
    // and a base_url that points at the proxy (so the in-box oaita dials the
    // UDS rather than the real upstream). The FUSE overlay substitutes this
    // file's bytes over the host oaita.toml's location for `--api` boxes —
    // see overlay::api_oaita_substitute. Best-effort: a missing host config
    // just writes a blank file; gen will then error with "no model set".
    let host_cfg = oaita::config::Config::load();
    let safe_toml = format!(
        "# Auto-generated by sarun for --api boxes. Read-only via FUSE.\n\
         model = {model:?}\n\
         base_url = \"http://oaita-proxy/v1\"\n",
        model = host_cfg.model.as_deref().unwrap_or(""));
    let _ = std::fs::write(paths::api_box_oaita_toml_path(), &safe_toml);
