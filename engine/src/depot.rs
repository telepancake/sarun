...
/// Convert a git-mirror VIEW (gitdepot shape: file nodes carry octal git
/// mode strings, directories are bare interior nodes) into a full layer
/// in the engine's attr dialect (decimal `mode` + `mtime` on every node),
/// nested under slash-separated `prefix` (`""` = box root). Symlinks
/// (git 120000) become engine symlink nodes; a gitlink (160000) becomes
/// a plain read-only pin file holding the submodule commit id — we do
/// not hold that repo.
pub fn layer_from_git_view(view: &depot_model::View, prefix: &str)
    -> Result<depot_model::Layer, String>
{
    use depot_model::{Attrs, BlobOp, Node};
    fn dir_attrs() -> Attrs {
        Attrs::from([(b"mode".to_vec(), a(0o040755u32)),
                     (b"mtime".to_vec(), a(0))])
    }
    fn conv(v: &depot_model::View, at: &str) -> Result<Node, String> {
        let mut node = Node::keep();
        if let Some(bytes) = &v.blob {
            let m = v.attrs.get(b"mode".as_slice())
                .ok_or_else(|| format!("{at}: git file without mode"))?;
            let s = std::str::from_utf8(m)
                .map_err(|_| format!("{at}: non-utf8 git mode"))?;
            let gm = u32::from_str_radix(s, 8)
                .map_err(|_| format!("{at}: bad git mode {s:?}"))?;
            let mode = match gm & 0o170000 {
                0o120000 => 0o120777,
                0o160000 => 0o100644, // gitlink pin file
                _ => gm,              // 100644 / 100755
            };
            node.blob = BlobOp::Set(bytes.clone());
            node.attrs = Some(Attrs::from([
                (b"mode".to_vec(), a(mode)),
                (b"mtime".to_vec(), a(0)),
            ]));
        } else {
            node.attrs = Some(dir_attrs());
        }
        for (name, child) in &v.children {
            let cn = format!("{at}/{}", String::from_utf8_lossy(name));
            node.children.insert(name.clone(), conv(child, &cn)?);
        }
        Ok(node)
    }
    let mut tree = conv(view, prefix)?;
    let mut root = Node::keep();
    if prefix.is_empty() {
        // The view root is the box root: children only, no "" row.
        root.children = std::mem::take(&mut tree.children);
    } else {
        let mut cur = &mut root;
        let mut it = prefix.split('/').peekable();
        while let Some(seg) = it.next() {
            if seg.is_empty() {
                return Err(format!("bad prefix {prefix:?}"));
            }
            let entry = cur.children.entry(seg.as_bytes().to_vec())
                .or_insert_with(Node::keep);
            if it.peek().is_none() {
                *entry = tree;
                break;
            }
            entry.attrs = Some(dir_attrs());
            cur = entry;
        }
    }
    Ok(depot_model::Layer { root })
