...
//
// IN-PROCESS COREUTILS: Bumba installs its injected-I/O uutils adapters as
// brush builtins — `cat cp mv rm mkdir wc sort cut tr …` run IN-PROCESS.
// Shell builtins WIN for overlapping names
// (coreutils installed first, BashMode set overwrites overlaps). Each coreutil runs
// inline under Bumba's scoped runtime guards — no process-fd dup2, pipeline-safe.
...
/// coreutils come from Bumba and are registered unconditionally. Scoped uucore
/// bindings select a bundle cached by utility and locale on the current thread,
/// so distinct or nested utils cannot poison one another's localization and do
/// not need disposable OS threads.
...
/// Look up the parser observation function carried by the ordinary Brush
