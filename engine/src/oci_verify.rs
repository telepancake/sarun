    ...
    pub fn load() -> Self {
        ...
        for e in pf.verify {
            let pem = match (&e.key, &e.key_file) {
                ...
                (None, None) => {
                    eprintln!(
                        "sarun oci: cosign verify entry for '{}' has no \
                               key/key_file",
                        e.match_
                    );
                    continue;
            ...
            match VerifyingKey::from_public_key_pem(pem.trim()) {
                ...
                Err(err) => eprintln!(
                    "sarun oci: cosign key for '{}' is not a \
                                       valid P-256 public key: {err}",
                    e.match_
                ),
            }
        }
...
pub fn verify(key: &VerifyingKey, manifest_digest: &str, sigs: &[CosignSig]) -> Result<(), String> {
    use base64::{Engine as _, prelude::BASE64_STANDARD};
    if sigs.is_empty() {
        return Err("no cosign signature found for this image".to_string());
    }
    for sig in sigs {
        let Ok(raw) = BASE64_STANDARD.decode(sig.signature_b64.trim()) else {
            continue;
        };
